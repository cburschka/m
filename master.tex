...
\end{lem}
Sei $S\subseteq\mathbf{FIN}\left(\left\{ E\right\} \right)$ die Klasse
der Graphen $\mathfrak{A}=\left(A,E^{\mathfrak{A}}\right)$ für die
gilt: $A$ enthält mindestens $\left\lceil \mathrm{log}\left|A\right|\right\rceil $
viele nicht-isolierte Knoten.
\begin{claim}
Es existiert eine $\mathrm{inv}\left(\mathrm{FO}\oplus\mathbf{ARB}\right)$-Formel,
die $S$ definiert.
