...
eine $\mathrm{IFP}\left[\sigma\cup\left\{ R\right\} \right]$-Formel
$\psi$, ein Tupel $\bar{x}\in\mathbf{var}^{k}$ und ein $k$-Tupel
$\bar{y}$ von $\mathrm{FP}\left[\sigma\right]$-Termen ist $\varphi=\left[\mathrm{ifp}_{R,\bar{x}}\psi\right]\left(\bar{y}\right)$
...
definieren wir eine Sequenz von $\left\{ R\right\} $-Strukturen $\left(\mathfrak{R}^{\left(i\right)}\right)_{i\in\mathbb{N}}$
und die Relationen $\left(R^{\left(i\right)}\right)_{i\in\mathbb{N}}$
wie folgt: 
\begin{eqnarray*}
R^{\left(0\right)} & \coloneqq & \emptyset\\
\mathfrak{R}^{\left(i\right)} & \coloneqq & \left(A,R^{\left(i\right)}\right)\\
R^{\left(i+1\right)} & \coloneqq & R^{\left(i\right)}\cup\psi^{\beta_{\backslash\bar{x}}}\left(\mathfrak{A}\right)
\end{eqnarray*}
Das heißt: $R^{\left(i+1\right)}$ fügt das Anfrageergebnis hinzu,
das unter der partiellen Belegung $\beta_{\backslash\bar{x}}$ entsteht,
wenn das Symbol $R$ durch die Relation $R^{\left(i\right)}$ interpretiert
wird.

Da $R^{\left(i\right)}\subseteq R^{\left(i+1\right)}\subseteq A^{k}$,
existiert ein Fixpunkt $R^{\left(\infty\right)}=R^{\left(\left|A\right|^{k}\right)}$.
Nun wird $\varphi$ wie folgt auswertet:

\begin{eqnarray*}
\mathfrak{A}\models\left(\left[\mathrm{ifp}_{R,\bar{x}}\psi\right]\left(\bar{y}\right)\right)^{\beta} & \Leftrightarrow & \left\llbracket \bar{y}\right\rrbracket \left(\mathfrak{A},\beta\right)\in R^{\left(\infty\right)}
\end{eqnarray*}
...
gerade Größe hat. Bekanntlich ist diese $\sigma$-Eigenschaft weder
durch $\mathrm{FO}\left[\sigma\right]$ noch $\mathrm{FP}\left[\sigma\right]$
auf geordneten oder ungeordneten Strukturen definierbar {[}CITE{]}:
...
\mathcal{C}\left[\mathfrak{A}\right]\left(g\right) & \coloneqq & \begin{cases}
1 & \mathrm{falls}\,\left(j_{0},j_{1}\right)\in\phi\\
0 & \mathrm{sonst}
\end{cases}
\end{eqnarray*}
...
Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathrm{var}\left(\varphi\right)}$
berechnet wird.
\end{lem}
...
Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert \cdot\left(2n+1\right)^{\mathrm{var}\left(\varphi\right)}$
berechnet wird.\end{lem}
\begin{proof}
