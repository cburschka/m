...
mit disjunkten Definitionsbereichen sei $\pi\oplus\pi':A\cup A'\rightarrow B\cup B'$
die folgende Abbildung:
\[
\pi\oplus\pi'\left(x\right)\coloneqq\begin{cases}
...
Wir formalisieren die Interpretation als eine Abbildung $\square^{\mathfrak{A}}:\sigma\rightarrow\mathcal{P}\left(A^{*}\right)$,
die jedes Symbol eine Relation der entsprechenden Stelligkeit zuweist.
...
die Menge aller $\sigma$-Strukturen mit dem Universum $U$.
\item Für $\mathbb{N}_{>0}$ sei $\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$
...
\item Für $\dot{\leqslant}\notin\sigma$ und $a,b\in\mathbb{N}$ seien $\mathbf{FIN}_{<}^{\left[a,b\right]}\left(\sigma\right)\subseteq\mathbf{FIN}^{\left[a,b\right]}\left(\sigma\cup\left\{ \leqslant\right\} \right)$
die endlichen $\sigma\cup\left\{ \leqslant\right\} $-Strukturen über
$\left[a,b\right]$, wobei $\leqslant$ die natürliche Ordnung von
$\left[a,b\right]$ ist.
\end{enumerate}
...
\end{eqnarray*}
Für eine Relation $R^{\mathfrak{A}}\subseteq A^{k}$ sei $\left[R^{\mathfrak{A}}\right]:A^{k}\rightarrow\left\{ 0,1\right\} $
eine Funktion mit $\left[R^{\mathfrak{A}}\right]\bar{t}\coloneqq\begin{cases}
1 & \mathrm{falls}\,t\in R^{\mathfrak{A}}\\
0 & \mathrm{sonst}
\end{cases}$.
...
Wenn $\sigma_{1}\cap\sigma_{2}=\emptyset$, so ist $\mathfrak{A}\cup\mathfrak{B}$
...
Wenn $\sigma_{1}=\sigma_{2}=\sigma$, so ist $\mathfrak{A}\cup\mathfrak{B}$
...
\begin{defn}
Es sei $\mathtt{MF}\left(\varphi\right)$ die maximale Anzahl freier
Variablen jeder Teilformel von $\varphi$: 
\begin{eqnarray*}
\mathtt{MF}\left(\varphi\right) & \coloneqq & \max\left\{ \mathrm{ar}\left(\psi\right)\mid\psi\,\mbox{ist Teilformel von}\,\varphi\right\} 
\end{eqnarray*}
\end{defn}
\begin{rem*}
Die Zahl $\mathtt{MF}\left(\varphi\right)$ ist ein Indikator für
die Anzahl der maximal benötigten Variablen $\mathrm{var}\left(\varphi\right)$
einer Formel, da gebundene Variablen umbenannt und wiederverwendet
werden können.
\end{rem*}
...
\end{eqnarray*}
\[
\mathtt{MF}\left(R\bar{x}\right)=\max\left(\max_{i=1}^{k}\mathtt{M}\left(x_{i}\right),\mathrm{frei}\left(R\bar{x}\right)\right)
\]
\begin{eqnarray*}
...
\end{eqnarray*}
\[
\mathtt{MF}\left(x_{1}\dot{=}x_{2}\right)=\max\left(\mathtt{MF}\left(x_{1}\right),\mathtt{MF}\left(x_{2}\right),\mathrm{frei}\left(x_{1}\dot{=}x_{2}\right)\right)
\]
\begin{eqnarray*}
...
\end{eqnarray*}
\[
\mathtt{MF}\left(R\bar{x}\right)=\max\left(\max_{i=1}^{k}\mathtt{MF}\left(\varphi_{i}\right),\mathrm{frei}\left(\varphi_{1}\gamma\cdots\gamma\varphi_{k}\right)\right)
\]
\begin{eqnarray*}
...
\end{eqnarray*}
\[
\mathtt{MF}\left(Qx\varphi\right)=\max\left(\mathtt{MF}\left(\varphi\right),\mathrm{frei}\left(Qx\varphi\right)\right)
\]
\begin{eqnarray*}
...
Ist dies nicht der Fall, so ist $Qx\varphi\equiv\varphi$.)\end{lyxlist}
\begin{rem*}
Im folgenden stehe $Q\bar{x}\psi$ und $\bar{x}=\bar{y}$ abgekürzt
...
Wir definieren die Klasse der $P$-positiven Formeln für $P\in\sigma$
wie folgt:\end{defn}
\begin{enumerate}
\item Jede $\mathrm{FO}\left[\sigma\backslash\left\{ P\right\} \right]$-Formel
...
$X$-positive $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel
$\psi$, ein Tupel $\bar{x}\in\mathbf{var}^{k}$ und ein $k$-Tupel
$\bar{y}$ von $\mathrm{\mathrm{FO}+\mathrm{LFP}}\left[\sigma\right]$-Termen
ist $\varphi=\left[\mathrm{lfp}_{X,\bar{x}}\psi\right]\left(\bar{y}\right)$
eine $\mathrm{\mathrm{FO}+\mathrm{IFP}}\left[\sigma\right]$-Formel.
\begin{eqnarray*}
...
$\mathfrak{A}$, eine $R$-positive $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel,
eine Belegung $\beta:\mathrm{frei}\left(\varphi\right)\backslash\bar{x}\rightarrow A$
und eine Relation $X\subseteq A^{k}$ definieren wir die $\left\{ R\right\} $-Struktur
$\mathfrak{R}_{X}$ und die Abbildung $F:\mathcal{P}\left(A^{k}\right)\rightarrow\mathcal{P}\left(A^{k}\right)$
wie folgt:
\begin{eqnarray*}
...
X
\end{array}\right)\right)\\
F\left(X\right) & \coloneqq & \varphi^{\beta}\left(\mathfrak{A}\oplus\mathfrak{R}_{X}\right)
\end{eqnarray*}
Das heißt: $F\left(X\right)$ ist das Anfrageergebnis von $\varphi^{\beta}$,
wenn das Symbol $R$ durch die Relation $X$ interpretiert wird.
...
Die Formel prüft nun, ob die Belegung des Tupels $\bar{y}$ im Fixpunkt
$F^{\infty}$ liegt: 
\begin{eqnarray*}
\left\llbracket \left[\mathrm{lfp}_{X,\bar{x}}\psi\right]\left(\bar{y}\right)\right\rrbracket \left(\mathfrak{A},\beta\right) & = & \left\llbracket X\bar{y}\right\rrbracket \left(\mathfrak{A}\oplus\mathfrak{R}_{F^{\infty}},\beta\right)
\end{eqnarray*}
...
Wir betrachten zwei äquivalente Definitionen von Zähloperatoren auf
Logiken mit disjunktem Orakel:
\begin{defn}
\textbf{Zählterm (wie \cite{anderson_et_al:LIPIcs:2014:4445})}

Der Zählterm ist eine zusätzlicher Termproduktion, die die erfüllenden
...
Die $\mathscr{L}+\Upsilon+\#$-Logik erweitert die $\mathscr{L}+\Upsilon$-Logik
um die folgende Regel:
\begin{lyxlist}{00.00.0000}
\item [{TC}] Für eine $\left(\mathscr{L}+\Upsilon+C\right)\left[\sigma\right]$-Formel
$\varphi$ und eine Variable $x\in\mathbf{var}$ ist $\#x\varphi$
ein $\left(\mathscr{L}+\Upsilon+C\right)\left[\sigma\right]$-Term.
\begin{eqnarray*}
\mathrm{frei}\left(\#xy\varphi\right)=\mathrm{frei}\left(\varphi\right)\backslash\left\{ x\right\}  &  & \mathrm{var}\left(\#_{x}\varphi\right)=\mathrm{var}\left(\varphi\right)\cup\left\{ x\right\} 
\end{eqnarray*}



Auf einer endlichen Struktur $\mathfrak{A}\in\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$
mit einer Belegung $\beta:\mathrm{frei}\left(\varphi\right)\backslash\left\{ x\right\} \rightarrow A\uplus\left[0,n\right]$
ist 
\[
\left\llbracket \#x\varphi\right\rrbracket \left(\mathfrak{A},\beta\right)\coloneqq\left|\left\{ a\in A\mid\mathfrak{A}\models\varphi^{\beta\oplus\binom{x}{a}}\right\} \right|
\]
...
\begin{defn}
\textbf{Zählquantor (wie \cite{Schweikardt2002}, aber mit disjunktem
Orakel)}

Der Zählquantor ist eine zusätzlicher Quantor, der die erfüllenden
...
Die $\mathscr{L}+\Upsilon+\exists^{C}$-Logik erweitert die $\mathscr{L}+\Upsilon$-Logik
um die folgende Regel:
\begin{lyxlist}{00.00.0000}
\item [{QC}] Für eine $\left(\mathscr{L}+\Upsilon+\exists^{C}\right)\left[\sigma\right]$-Formel
$\varphi$ und zwei Variablen $x,y\in\mathbf{var}$ ist $\exists^{=y}x\varphi$
eine $\left(\mathscr{L}+\Upsilon+\exists^{C}\right)\left[\sigma\right]$-Formel.
\begin{eqnarray*}
\mathrm{frei}\left(\exists^{=y}x\varphi\right)=\left(\mathrm{frei}\left(\varphi\right)\backslash\left\{ x\right\} \right)\cup\left\{ y\right\}  &  & \mathrm{var}\left(\#_{x}\varphi\right)=\mathrm{var}\left(\varphi\right)\cup\left\{ x,y\right\} 
\end{eqnarray*}
...
mit einer Belegung 
\[
\beta:\left(\mathrm{frei}\left(\varphi\right)\backslash\left\{ x\right\} \right)\cup\left\{ y\right\} \rightarrow A\uplus\left[0,n\right]
\]
 gilt: 
\[
\mathfrak{A}\models\left(\exists^{=y}x\varphi\right)^{\beta}\Leftrightarrow\left|\left\{ a\in A\mid\mathfrak{A}\models\varphi^{\beta\oplus\binom{x}{a}}\right\} \right|=\beta y
\]
...
\end{defn}
\begin{fact}
Die Logiken $\mathscr{L}+\Upsilon+\#$ und $\mathscr{L}+\Upsilon+C$
sind äquivalent:

\begin{eqnarray*}
\exists^{=y}x\psi & \equiv & \left(y\dot{=}\#x\psi\right)\\
\#x_{1}\psi_{1}\dot{=}\#x_{2}\psi_{2} & \equiv & \exists z\left(\exists^{=y}x_{1}\psi_{1}\wedge\exists^{=y}x_{2}\psi_{2}\right)\\
R\left(\#x_{1}\psi_{1}\cdots\#x_{k}\psi_{k}\right) & \equiv & \exists\bar{z}\left(R\bar{z}\wedge\bigwedge_{i=1}^{k}\exists^{=y_{i}}x_{i}\psi_{i}\right)
\end{eqnarray*}


(Für neue Variablen $z,z_{1},\cdots z_{\mathrm{ar}\left(R\right)}\in\mathbf{var}$.)
Die Einführung dieser neuen Variablen ändert $\mathtt{MF}\left(\varphi\right)$
höchstens um die Konstante $\max_{R\in\sigma}\left(\mathrm{ar}\left(R\right)\right)$.
\end{fact}
...
$\bar{t}\in U^{k}$ sei die Ausgabe von $\mathcal{C}$:
\[
...
einen Automorphismus $\hat{\pi}:G\rightarrow G$ des Graphen $\left(G,W\right)$
induziert, der die folgenden Bedingungen erfüllt:
\begin{enumerate}
\item $\hat{\pi}\left(W\right)=W$
\item Für alle nicht-konstanten Inputs $g$ mit $\Sigma\left(g\right)=R\bar{x}$
gilt $\Sigma\left(\hat{\pi}g\right)=R\,\bar{x'}$, mit $\bar{x'}=\pi\bar{x}$.
\item Für alle übrigen Gates gilt $\Sigma\left(\hat{\pi}g\right)=\Sigma\left(g\right)$.
\item Für jedes Tupel $\bar{x}\in U^{k}$ gilt $\hat{\pi}\Omega\left(\bar{x}\right)=\Omega\left(\pi x\right)$.\end{enumerate}
\begin{lem}
...
sei die disjunkte Vereinigung der Anfragen $q_{\mathcal{C}_{n}}$
jedes Schaltkreises: 
\[
q_{\bar{\mathcal{C}}}\coloneqq\biguplus_{n\in\mathbb{N}}q_{\mathcal{C}_{n}}
\]
...
und polynomieller Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert n^{\mathtt{MF}\left(\varphi\right)}$
...
\[
\begin{array}{ccccc}
\left\llbracket R\bar{x}\right\rrbracket \left(\mathfrak{A},\bar{t}\right)=1 & \overset{\mathrm{def}}{\Leftrightarrow} & \bar{t}\in R^{\mathfrak{A}} & \overset{\mathrm{def}}{\Leftrightarrow} & \mathcal{C}_{n}^{\varphi}\left[\mathfrak{A}\right]\left(\Omega\left(\bar{t}\right)\right)=1\end{array}
\]
...
\[
\begin{array}{ccccc}
\left\llbracket y_{1}\dot{=}y_{2}\right\rrbracket \left(\mathfrak{A},\bar{t}\right)=1 & \overset{\mathrm{def}}{\Leftrightarrow} & \beta_{\bar{t}}y_{1}=\beta_{\bar{t}}y_{2} & \overset{\mathrm{def}}{\Leftrightarrow} & \mathcal{C}_{n}^{\varphi}\left[\mathfrak{A}\right]\left(\Omega\left(\bar{t}\right)\right)=1\end{array}
\]
...
\begin{eqnarray*}
\hat{\pi}W_{\wedge}=\left\{ \left(\Omega_{i}\left(\rho_{i}\bar{t}\right),g_{\bar{t}}\right)\mid1\leqslant i\leqslant m,\,\bar{t}\in U^{k}\right\}  & = & \left\{ \left(\hat{\pi}\Omega_{i}\left(\rho_{i}\bar{t}\right),\hat{\pi}g_{\bar{t}}\right)\mid1\leqslant i\leqslant m,\,\bar{t}\in U^{k}\right\} \\
 & = & \left\{ \left(\hat{\pi}_{i}\Omega_{i}\left(\rho_{i}\bar{t}\right),g_{\pi\bar{t}}\right)\mid1\leqslant i\leqslant m,\,\bar{t}\in U^{k}\right\} \\
 & = & \left\{ \left(\Omega_{i}\left(\rho_{i}\pi\bar{t}\right),g_{\pi\bar{t}}\right)\mid1\leqslant i\leqslant m,\,\bar{t}\in U^{k}\right\} \\
 & = & W_{\wedge}
\end{eqnarray*}
...
\end{cases}
\]
Es gilt $\hat{\pi}W_{\neg}=\left\{ \left(\hat{\pi}\Omega'\left(\bar{t}\right),\hat{\pi}g_{\bar{t}}\right)\mid\bar{t}\in U^{k}\right\} =\left\{ \left(\Omega'\left(\pi\bar{t}\right),g_{\pi\bar{t}}\right)\mid\bar{t}\in U^{k}\right\} =W_{\neg}$.
\end{description}
...
beschränkte Tiefe und eine durch $5\left\Vert \varphi\right\Vert \cdot n^{\mathtt{MF}\left(\varphi\right)}$
...
Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}$
berechnet wird.
\end{lem}
(Dieses Lemma bildet die erste Hälfte des Theorems \ref{thm:5.5:fo-ord}.)
\begin{proof}
Sei $\varphi$ eine $k$-stellige $\mathrm{FO}+\mathbf{ORD}\left[\sigma\right]$-Formel,
und $\mathfrak{A}\in\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$.
Diese wird auf der vereinigten $\left(\sigma\cup\left\{ \leqslant\right\} \right)$-Struktur
$\mathfrak{A}\cup\Upsilon\left(n\right)$ ausgewertet, wobei $\Upsilon\left(n\right)\coloneqq\left(\left[0,n\right],\leqslant\right)$.

Der Schaltkreis $\mathcal{C}_{n}$ wird aber auf einer $\sigma$-Struktur
...
$\Upsilon\left(n\right)$ ist! Daher wird das Universum $\left[0,n\right]$
von $\Upsilon\left(n\right)$ durch eine Umbenennung auf $\left[n+1,2n+1\right]$
,,verschoben``:
\begin{eqnarray*}
...
Nun ist $\mathfrak{A}'\cup\Upsilon'\left(n\right)\in\mathbf{FIN}^{\left[1,2n+1\right]}\left(\sigma\cup\left\{ \leqslant\right\} \right)$
(wobei die Ordnung $\leqslant$ weiterhin nur auf $\left[n+1,2n+1\right]$
definiert ist).

Weil $\Upsilon\left(n\right)\cong\Upsilon'\left(n\right)$ und $\mathfrak{A}\cong\mathfrak{A}'$,
und beide Strukturen disjunkt voneinander sind, ist auch $\mathfrak{A}\cup\Upsilon\left(n\right)\cong\mathfrak{A}'\cup\Upsilon'\left(n\right)$.
Daher gilt: 
\[
\left\llbracket \varphi\right\rrbracket \left(\mathfrak{A}\cup\Upsilon\left(n\right)\right)=\left\llbracket \varphi\right\rrbracket \left(\mathfrak{A}'\cup\Upsilon'\left(n\right)\right)
\]
.
...
dem vorhergehenden Beweis für $\mathrm{FO}$. Dieser hat offensichtlich
die geforderte Größe $S_{n}=S_{2n+1}'\leqslant5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}$.

Anschließend konstruieren wir daraus den $\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis
$\dot{\mathcal{C}_{n}^{\varphi}}=\left(G,W,\dot{\Sigma},\dot{\Omega},U\right)$
über $U=\left[1,n\right]$, indem alle nicht-konstanten Inputs $\Sigma\left(g\right)=R\bar{t}$
mit $\bar{t}\notin U^{k}$ oder $R\notin\sigma$ durch Konstanten
$\dot{\Sigma}\left(g\right)\in\left\{ \mathbf{0},\mathbf{1}\right\} $
ersetzt werden, und die Ausgangsfunktion auf $\dot{\Omega}=\Omega_{\mid\dot{U}}$
reduziert wird.
\begin{casenv}
...
Automorphismus in $\dot{\mathcal{C}_{n}^{\varphi}}$ ist. Dazu muss
nur $\dot{\Sigma}$ und $\dot{\Omega}$ betrachtet werden, da der
Graph $\left(G,W\right)$ unverändert bleibt.
\begin{enumerate}
\item Für $\bar{t}\in U^{k}$ \end{enumerate}
\begin{casenv}
...
\begin{eqnarray*}
\mathtt{MF}\left(\hat{\varphi}\right) & = & \max\left(\mathtt{MF}\left(\psi_{1}\right),\mathtt{MF}\left(\psi_{2}\right),1+\left|\mathrm{frei}\left(\#y_{1}\psi_{1}\right)\cup\mathrm{frei}\left(\#y_{1}\psi_{1}\right)\right|\right)\\
 & = & \max\left(\mathtt{MF}\left(\psi_{1}\right),\mathtt{MF}\left(\psi_{2}\right),1+\mathrm{ar}\left(\#y_{1}\psi_{1}=\#y_{1}\psi_{2}\right)\right)\\
 & \leqslant & 1+\mathtt{MF}\left(\varphi\right)
\end{eqnarray*}
...
Die Fixpunktlogik $\mathrm{FO+LFP}\left[\sigma\right]$ ist echt ausdrucksstärker
...
definierbar \cite{Immerman1982,Libkin2012}. In $\mathrm{FO+FP}\left[\left\{ E\right\} \right]$
wird diese Klasse durch die folgende Formel definiert:
\[
\varphi\left(u,v\right)\coloneqq\left[\mathrm{lfp}_{R,\left(x,y\right)}\,\left(\exists z\,\left(E\left(x,z\right)\wedge R\left(z,y\right)\right)\vee x=y\right)\right]\left(u,v\right)
\]
 Trotzdem kann jede $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel
$\varphi$ durch eine Familie von $\mathrm{FO}\left[\sigma\right]$-Formeln
$\left(\varphi_{n}\right)_{n\in\mathbb{N}}$ mit polynomieller Länge
$\left\Vert \varphi_{n}\right\Vert \leqslant n^{\left\Vert \varphi\right\Vert }$
...
eine Schaltkreisfamilie erzeugt werden, die $\varphi$ berechnet.\marginpar{CITE k-Variablen Fragment infinitäre Logik, u.a. {[}LogKomp 2007{]}.}
\begin{eqnarray*}
...
\begin{lem}
Für jede $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel $\varphi$
und jede Größe $n\in\mathbb{N}$ existiert eine $\mathrm{FO}\left[\sigma\right]$-Formel
$\hat{\varphi}$ mit höchstens $2\left\Vert \varphi\right\Vert $
Variablen und $\left\Vert \hat{\varphi}\right\Vert \leqslant n^{\left\Vert \varphi\right\Vert }$,
so dass $\models_{n}\left(\hat{\varphi}\leftrightarrow\varphi\right)$.\end{lem}
\begin{proof}
Die Konstruktion erfolgt induktiv über die Definition von $\mathrm{FO}+\mathrm{LFP}$.
Sei $\varphi$ eine $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel
für eine Signatur $\sigma$.

Die Tatsache, dass jeder $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Term
$t$ eine Variable $t\in\mathbf{var}$ ist, macht die atomaren Fälle
trivial.
\begin{casenv}
\item Falls $\varphi=R\bar{x}$ für ein $k$-Tupel von $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Termen
$x$ und $R/k\in\sigma$, so ist $\varphi$ bereits eine $\mathrm{FO}\left[\sigma\right]$-Formel.
\item Falls $\varphi=x_{1}\dot{=}x_{2}$ für zwei $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Terme
$x_{1},x_{2}$, so ist $\varphi$ bereits eine $\mathrm{FO}\left[\sigma\right]$-Formel.
\item Falls $\varphi=\left(\varphi_{1}\gamma\cdots\gamma\varphi_{k}\right)$
für $k$ $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formeln $\varphi_{1},\cdots,\varphi_{k}$
und $\gamma\in\left\{ \wedge,\vee,\rightarrow,\leftrightarrow\right\} $,
so existieren per Annahme die dazu $n$-äquivalenten $\mathrm{FO}\left[\sigma\right]$-Formeln
$\hat{\varphi_{1}},\cdots,\hat{\varphi_{k}}$:
\[
\hat{\varphi}\coloneqq\left(\hat{\varphi_{1}}\gamma\cdots\gamma\hat{\varphi_{k}}\right)
\]

\item Falls $\varphi=\neg\psi$ für eine $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel
$\psi$, so existiert per Annahme die $n$-äquivalente Formel $\hat{\psi}$:
\begin{eqnarray*}
\hat{\varphi} & \coloneqq & \neg\hat{\psi}
\end{eqnarray*}

\item Falls $\varphi=Qx\psi$ für $Q\in\left\{ \exists,\forall\right\} $,
$x\in\mathbf{var}$ und eine $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$-Formel
$\psi$ , so existiert per Annahme die $n$-äquivalente Formel $\hat{\psi}$:
\begin{eqnarray*}
\hat{\varphi} & \coloneqq & Qx\hat{\psi}
\end{eqnarray*}

\item Falls $\varphi=\left[\mathrm{lfp}_{X,\bar{x}}\psi\right]\left(\bar{y}\right)$
für eine $k$-stellige Relationsvariable $X\in\mathbf{var}_{2}\backslash\sigma$,
$\bar{x}\in\mathbf{var}^{k}$, eine $\mathrm{FO}+\mathrm{LFP}\left[\sigma\cup\left\{ X\right\} \right]$-Formel
$\psi$ und ein $k$-Tupel von $\mathrm{FO+LFP}\left[\sigma\right]$-Termen
$\bar{y}$, so gilt:


Per Induktionsannahme existiert eine $\mathrm{FO}\left[\sigma\right]$-Formel
$\hat{\psi}$ mit $\models_{n}\left(\hat{\psi}\leftrightarrow\psi\right)$.


Sei die Sequenz $\left(\hat{\psi}^{\left(i\right)}\right)_{i\in\mathbb{N}}$
induktiv wie folgt induktiv definiert. Sei $\bar{u}\in\mathbf{var}^{k}\backslash\mathrm{frei}\left(\psi\right)$
ein Tupel von Variablen, die nicht frei in $\psi$ vorkommen.
\begin{eqnarray*}
\hat{\psi}^{\left(0\right)}\left(\bar{x}\right) & \coloneqq & \neg x_{1}\dot{=}x_{1}\\
\chi^{\left(i\right)}\left(\bar{z}\right) & \coloneqq & \exists\bar{u}\left(\bar{u}=\bar{z}\wedge\exists\bar{x}\left(\bar{x}=\bar{u}\wedge\hat{\psi}^{\left(i\right)}\left(\bar{x}\right)\right)\right)\\
\hat{\psi}^{\left(i+1\right)} & \coloneqq & \left(\hat{\psi}^{\left(i\right)}\right)^{\left(\frac{R\bar{z}}{\chi^{\left(i\right)}\left(\bar{z}\right)}\right)}
\end{eqnarray*}
Hierbei sei $\psi^{\left(\frac{R\bar{x}}{\varphi\left(\bar{x}\right)}\right)}$
die Formel, die aus $\psi$ entsteht, indem alle Atome $R\bar{x}$
durch $\varphi\left(\bar{x}\right)$ ersetzt werden. Der Zwischenschritt
über $\bar{u}$ ist notwendig, weil $\bar{z}$ und $\bar{x}$ zum
Teil die gleichen Variablen enthalten können, und daher ihr Geltungsbereich
nicht überlappen darf.


Nun gelte:
\[
\hat{\varphi}\coloneqq\exists\bar{u}\left(\bar{u}=\bar{y}\wedge\hat{\psi}^{\left(n^{k}\right)}\right)
\]
...
Wir weisen induktiv nach, dass $\hat{\psi}^{\left(i\right)}\left(\mathfrak{A}\right)=F^{i}\left(\emptyset\right)$ 
