...
und $\mathrm{FP}\left(\mathbf{ARB}\right)$ charakterisiert. $\mathrm{FO}\left(\mathbf{ARB}\right)$
...
und $\mathcal{L}+\Upsilon+\exists^{\geqslant}$ sind äquivalent, modulo
eines festen Zuwachses in $\left\Vert \varphi\right\Vert $ und $\left|\mathrm{var}\left(\varphi\right)\right|$.\end{prop}
\begin{proof}
...
$\bar{\mathcal{C}}$ $\mathcal{K}$-\textbf{uniform}, wenn ein $n_{0}\in\mathbb{N}$
existiert, so dass eine $\mathcal{K}$-Turingmaschine bei Eingabe
des Worts $1\cdots1\in\left\{ 0,1\right\} ^{n}$ mit $n\geqslant n_{0}$
eine Repräsentation von $\mathcal{C}_{n}$ berechnet.
...
kann, ohne die Klasse der $\mathrm{LOGSPACE}$-uniformen symmetrischen
$\mathrm{AC}^{0}$-Schaltkreise zu verlassen.
\[
\mathrm{FO}+\mathbf{ORD}\subseteq\left(\mathrm{SAC}^{0}\right)^{\mathrm{LOGSPACE}}
\]

\begin{lem}
\label{lem:fo-ord-circuit}Jede $\left(\mathrm{FO}+\mathbf{ORD}\right)\left[\sigma\right]$-Formel
...
nur auf $\left[n+1,2n+1\right]$ definiert ist): 
\[
...
dem Beweis von Lemma \ref{lem:fo-circuit}. Dieser arbeitet korrekt
...
$\mathrm{LOGSPACE}$-uniforme Schaltkreisfamilie konstanter Tiefe,
die $q$ berechnet.
\end{proof}
\pagebreak{}
...
vorhergehende Konstruktion aus Lemma \ref{lem:fo-ord-circuit} angepasst
werden kann, indem Majority-Gates hinzugefügt werden.
\[
\mathrm{FO}+C\subseteq\left(\mathrm{SAC}^{0}+\mathbf{MAJ}\right)^{\mathrm{LOGSPACE}}
\]
...
wird nach dem gleichen Schema wie in Lemma \ref{lem:fo-ord-circuit},
...
symmetrisch (siehe Lemma \ref{lem:fo-ord-circuit}).
...
(insbesondere $\mathrm{FO}+\mathbf{ARB}$, welches alle diese Logiken
einschließt), und weisen nach, dass die Konstruktion aus \ref{lem:fo-ord-circuit}
auf beliebige Prädikate erweitert werden kann, wobei die $\mathrm{LOGSPACE}$-Uniformität,
aber nicht die beschränkte Größe geopfert wird.
\[
\mathrm{FO}+\mathbf{ARB}\subseteq\left(\mathrm{SAC}^{0}\right)^{P/\mathrm{poly}}
\]

\begin{lem}
\label{lem:fo+arb}Seien $\eta$ und $\sigma$ disjunkte Signaturen
...
$\mathcal{C}_{2n+1}^{\varphi}$ wie in Lemma \ref{lem:fo-ord-circuit}
erzeugt.
...
und daher $P/\mathrm{poly}$-uniform.
\end{proof}
\pagebreak{}
...
In diesem Abschnitt werden wir die bereits gezeigten Konstruktionen
auf Fixpunktlogik erweitern, die durch Schaltkreise beliebiger Tiefe
...
\subsection{Eine Sequenz von erststufigen Formeln}
...
Wir nutzen die Normalform und den Beweis von Lemma \ref{lem:lfp-inf}
aus, um aus einer $\mathrm{LFP}\left[\sigma\right]$-Formel $\varphi$
direkt einen $\left(\sigma,\mathbb{B}\right)$-Schaltkreis zu erzeugen,
der die richtige Tiefe und Größe hat. Wir formulieren die Konstruktion
für allgemeine Logiken $\mathcal{L}$ unter Voraussetzung einer entsprechenden
Schaltkreisklasse konstanter Tiefe.
\begin{lem}
...
\end{lem}
In Kombination mit dem Rest dieses Kapitels entsteht daraus eine Konstruktion
für $\mathrm{LFP}+\mathbf{ORD}$ (mit Lemma \ref{lem:fo-ord-circuit})
und $\mathrm{LFP}+\mathbf{ARB}$ (mit Lemma \ref{lem:fo+arb}), denn
$\mathrm{LOGSPACE}\subseteq P$ (siehe Abschnitt 4.3 in \cite{arora-barak}).
\begin{proof}
...
\subsection{Fixpunktlogik mit Zählquantoren}

Wir haben im letzten Abschnitt eine Normalform vorausgesetzt, diese
aber nur für die Logiken $\mathrm{LFP}$ und $\mathrm{LFP}+\Upsilon$
nachgewiesen. Den Spezialfall $\mathrm{LFP}+C$ betrachten wir gesondert,
und weisen nach, dass die Konstruktion aus Lemma \ref{lem:fo+c} mit
dem Fixpunkt-Operator kombinierbar ist.
...
$U=\left[1,n\right]$ und einen rigiden (siehe Definition \ref{def:rigid})
