...
\[
s'\left(n\right)\coloneqq s\left(2n+1\right)+\left(2n+\left(2n+1\right)^{\mathrm{var}\left(\varphi\right)}\right)\left\Vert \varphi\right\Vert \in\mathrm{poly}\left(n\right)
\]
...
Wir gehen analog zu Lemma \ref{lem:oracle-circuit} für $\mathcal{L}+\Upsilon$
vor, erweitern aber die Konstruktion aus der Voraussetzung um den
folgenden Schritt:
...
der $k$-stellige Schaltkreis für die Formel $\psi$:
\[
...
T\left(\mathcal{C}_{2n+1}^{\varphi}\right) & \leqslant & 1+T\left(\mathcal{C}_{2n+1}^{\psi}\right)\\
 & \overset{\mathrm{Ann.}}{\leqslant} & 1+\left\Vert \psi\right\Vert +t\left(2n+1\right)\leqslant\left\Vert \varphi\right\Vert +t\left(2n+1\right)
\end{eqnarray*}
\begin{eqnarray*}
\left|\mathcal{C}_{2n+1}^{\varphi}\right| & \leqslant & \left(2n+1\right)^{k}+2n+\left|\mathcal{C}_{2n+1}^{\psi}\right|\\
 & \overset{\mathrm{Ann.}}{\leqslant} & \left(2n+1\right)^{k}+2n+\left\Vert \psi\right\Vert \left(\left(2n+1\right)^{\mathrm{var}\left(\varphi\right)}+2n\right)+s\left(2n+1\right)\\
 & \leqslant & \left\Vert \varphi\right\Vert \left(\left(2n+1\right)^{\mathrm{var}\left(\varphi\right)}+2n\right)+s\left(2n+1\right)
\end{eqnarray*}
...
Wir haben im letzten Abschnitt eine Normalform vorausgesetzt, die
aber nur für die Logiken $\mathrm{LFP}$ und $\mathrm{LFP}+\Upsilon$
nachgewiesen ist. Den Spezialfall $\mathrm{LFP}+C$ betrachten wir
gesondert, und weisen nach, dass die Konstruktion aus Lemma \ref{lem:counting-circuit}
mit dem Fixpunkt-Operator kombinierbar ist.
\begin{eqnarray*}
...
\begin{lem}
Sei $\mathcal{L}+\Upsilon$ eine Logik und $\mathcal{K}$ eine Komplexitätsklasse
mit $P\subseteq\mathcal{K}$, so dass für jede $\left(\mathcal{L}+\Upsilon\right)\left[\sigma\right]$-Formel
eine $\mathcal{K}$-uniforme, symmetrische $\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreisfamilie
mit polynomiell beschränkter Tiefe und Größe $t\left(n\right),s\left(n\right)\in\mathrm{poly}\left(n\right)$
existiert, die die gleiche Anfrage beschreibt.

Dann existiert für jede $\left(\mathcal{L}+\Upsilon+C\right)\left[\sigma\right]$-Formel
$\varphi$ eine entsprechende $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreisfamilie \end{lem}
\begin{proof}
Induktiv über den Aufbau von $\varphi$:
\begin{casenv}
\item Falls $\varphi$ eine $\left(\mathcal{L}+\Upsilon\right)\left[\sigma\right]$-Formel
ohne Zählquantor ist, dann folgt die Behauptung direkt aus der Voraussetzung. 
\item Falls $\varphi=\exists^{\geqslant y}x$
\end{casenv}
\end{proof}
