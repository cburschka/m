...
notiert werden, um die Zuordnung zu verdeutlichen: 
\[
...
wird auf einer geordneten $\sigma$-Struktur $\mathfrak{A}\in\mathbf{FIN}_{\leqslant}\left(\sigma\right)$
...
\begin{proof}
Sei $\mathcal{C}$ ein symmetrischer $\left(\sigma,\mathbb{B}\right)$-Schaltkreis,
und $\mathfrak{A}$ eine beliebige $\sigma$-Struktur mit $\left|A\right|=\left|U\right|$.
Seien $\pi_{1}$ und $\pi_{2}$ zwei beliebige Bijektionen von $A$
auf $U$, und sei $\bar{a}\in A^{\mathrm{ar}\left(\mathcal{C}\right)}$
...
Hierbei ist zu beachten, dass $U=\left[1,n\right]$ disjunkt von dem
Universum $\left[0,n\right]$ der Orakelstruktur $\Upsilon\left(n\right)$
sein muss! Zur Unterscheidung wird das Universum $\left[0,n\right]$
umbenannt zu $N\coloneqq\left\{ \dot{0},\cdots,\dot{n}\right\} $
und $\Upsilon\left(n\right)\coloneqq\left(N,\leqslant\right)$ betrachtet.

Wir konstruieren aus der $\mathrm{FO}\left[\sigma\cup\left\{ \leqslant\right\} \right]$-Formel
zunächst einen $\left(\sigma\cup\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis
...
Daraus wird der $\left(\sigma\cup\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis
...
eine Permutation von $U=\left[1,2n+1\right]$ mit $\pi_{\mid\left[n+1,2n+1\right]}=\mathbf{id}$.
