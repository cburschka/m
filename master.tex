...
unter Umständen mehrere konstante Vorgänger. Zum Ausgleich erlauben
wir einen Multigraph:
\begin{defn}
\textbf{Multischaltkreis}

Ein $\left(\sigma,\mathbb{B}\right)$-Multischaltkreis $\mathcal{C}=\left(G,W,\Sigma,\Omega,U\right)$
ist analog zum $\left(\sigma,\mathbb{B}\right)$-Schaltkreis definiert,
aber $\left(G,W\right)$ bildet einen azyklischen Multigraph:
\[
W:G\times G\rightarrow\left[0,\left|G\right|\right]
\]
\[
\sum_{h\in G}W\left(h,g\right)\leqslant\left|G\right|\,\mathrm{f\ddot{u}r\,alle}\,g\in G
\]


(Die Anzahl der eingehenden Kanten sei ohne Beschränkung der Allgemeinheit
durch $\left|G\right|$ beschränkt.)

Für die Auswertung eines internen Gates gilt analog zu \ref{def:Auswertung-von-Schaltkreisen}:
\begin{eqnarray*}
j_{1} & \coloneqq & \sum_{h\in G}\left(\mathcal{C}\left[\mathfrak{A}\right]\left(h\right)\cdot W\left(h,g\right)\right)\\
j_{0} & \coloneqq & \sum_{h\in G}W\left(h,g\right)-j_{1}\\
\mathcal{C}\left[\mathfrak{A}\right]\left(g\right) & \coloneqq & \left[\Sigma\left(g\right)\right]\left(j_{0},j_{1}\right)
\end{eqnarray*}
\end{defn}
\begin{prop}
Jeder $\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis
kann in Polynomialzeit in einen rigiden Schaltkreis umgewandelt werden.
Jeder $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreis
kann in Polynomialzeit zu einem rigiden Multischaltkreis umgewandelt
werden.\end{prop}
\begin{proof}
D
\end{proof}
...
\end{defn}
Es wird nun Lemma \ref{lem:lemma-19} und \ref{lem:lemma-20} aus
dem letzten Abschnitt verwendet, um eine obere Schranke für den Träger
jedes Gates $g$ zu finden.
\begin{lem}
...
\begin{proof}
Falls der Orbit durch $s<n$ beschränkt ist, enthält $\mathcal{C}$
keine relationalen Inputs (weil jedes Input $g\in G$ mit $\Sigma\left(g\right)=R\bar{x}$
und $X=\left\{ x_{1},\cdots,x_{k}\right\} $ einen Orbit der Größe
$\mathrm{Orb}_{\mathcal{C}}\left(g\right)\geqslant n^{\left|X\right|}\geqslant n$
haben muss). In diesem Fall induziert jede Permutation den gleichen
Automorphismus $\mathbf{id}$, jedes Gate hat den Träger $\mathcal{P}_{\max}$
und es gilt $\mathrm{SP}\left(\mathcal{C}\right)=\left\Vert \mathcal{P}_{\max}\right\Vert =0$.
Daher betrachten wir nur Schaltkreise mit $s\geqslant n$.

Sei $g\in G$ beliebig.
\begin{casenv}
\item Wenn $\Sigma\left(g\right)\in\left\{ \mathbf{0},\mathbf{1}\right\} $,
dann ist auf Grund der Rigidität $\mathrm{Stab}_{\mathcal{C}}\left(g\right)=\mathrm{Sym}_{U}$,
und $\left\Vert \mathrm{SP}\left(g\right)\right\Vert =\left\Vert \mathcal{P}_{\max}\right\Vert =0$.
\item Wenn $\Sigma\left(g\right)=R\bar{x}$ für $R/k\in\sigma$ und $\bar{x}\in U^{k}$
mit $X=\left\{ x_{1},\cdots,x_{k}\right\} $, dann muss jede Permutation,
die die Elemente von $X$ fixiert, auf Grund der Rigidität einen Automorphismus
induzieren, der $g$ fixiert: 
\[
\mathrm{Stab}_{U}\left(X\right)\subseteq\mathrm{Stab}_{\mathcal{C}}\left(g\right)
\]
Es ist $\mathcal{P}_{X}$ ist ein Träger von $g$, und $\left\Vert \mathcal{P}_{X}\right\Vert =\left|X\right|\leqslant k$.
Daher ist $\left\Vert \mathrm{SP}\left(g\right)\right\Vert $ durch
die konstante Stelligkeit von $R$ beschränkt: 
\[
\left\Vert \mathrm{SP}\left(g\right)\right\Vert \leqslant\left\Vert \mathcal{P}_{X}\right\Vert \leqslant k
\]
Für hinreichend große $n$ ist $k\leqslant n^{1-\epsilon}\leqslant\frac{33}{\epsilon}\frac{\log s}{\log n}$.
\item Wenn $g$ ein internes Gate ist, dann nehmen wir an, $\mathrm{SP}\left(g\right)>\frac{n}{2}$
und $\mathrm{SP}\left(g'\right)\leqslant\frac{n}{2}$ für alle Vorgänger
$g'$ von $g$.
\[
k\coloneqq\left\lceil \frac{8}{\epsilon}\frac{\log s}{\log n}\right\rceil 
\]
Wir teilen das Universum $U$ in gleich große Teile der Größe $k+2$:
\begin{eqnarray*}
U & = & \biguplus_{i=1}^{k'}U_{i}\uplus\left[k'\left(k+2\right)+1,n\right]\\
U_{i} & \coloneqq & \left[\left(i-1\right)\cdot\left(k+2\right)+1,i\cdot\left(k+2\right)\right]\\
k' & \coloneqq & \left\lfloor \frac{n}{k'+2}\right\rfloor 
\end{eqnarray*}

\end{casenv}
\end{proof}
Sei $C$ ein symmetrischer Schaltkreis über $\left|U\right|=n$ mit
