...
\begin{lem}
\label{lem:counting-circuit}Sei $\mathcal{L}$ eine Logik, $k\in\mathbb{N}$
...
eine symmetrische, $\mathcal{K}$-uniforme $\left(\sigma,\mathbb{B}\right)$-Schaltkreisfamilie
mit $\mathcal{O}\left(n^{k\left\Vert \varphi\right\Vert }\right)$-Tiefe
und $\mathrm{poly}\left(n\right)$-Größe existiert, die die gleiche
Anfrage beschreibt.
...
mit der Tiefe $\mathcal{O}\left(n^{k\left\Vert \varphi\right\Vert }\right)$-Tiefe
und $\mathrm{poly}\left(n\right)$-Größe: 
\end{lem}
Insbesondere folgt aus der Kombination mit Lemma \ref{lem:fo-circuit}
und Lemma \ref{lem:oracle-circuit} (mit $k=0$ und der Tiefenbeschränkung
...
für $\psi$ gilt. Sei $\mathcal{C}_{2n+1}^{\psi}$ der symmetrische
Schaltkreis für die Formel $\psi$ mit der Tiefe $t\left(n\right)\in\mathcal{O}\left(n^{k\left\Vert \psi\right\Vert }\right)$
und der Größe $s\left(n\right)\in\mathrm{poly}\left(n\right)$.
\[
...
\end{eqnarray*}
Der neue Schaltkreis hat die Tiefe $t'\left(n\right)=t\left(n\right)+1\in\mathcal{O}\left(\right)$
...
des neuen Schaltkreises $t_{\varphi}\left(2n+1\right)+$, und seine
Größe ist in $\mathrm{poly\left(n\right)}$.
...
\section{Symmetrie ist eine echte Einschränkung von $\mathrm{AC}^{0}$}
