...
\usepackage{babel}
  ...
  \addto\captionsngerman{\renewcommand{\factname}{Fakt}}
 ...
 \addto\captionsenglish{\renewcommand{\casename}{Case}}
 \addto\captionsngerman{\renewcommand{\casename}{Fall}}
...
Eine Datenbank-Anfrage auf einem solchen Schema wird als Funktion
modelliert, die jede endliche Struktur auf eine Relation abbildet.

Von besonderem Interesse ist die Daten-Komplexität solcher Anfragen:
Die Komplexität der Auswertung einer festen Anfrage in Abhängigkeit
von der Größe der eingegebenen Struktur. Wir betrachten hierbei Klassen
von Anfragen, die durch boolesche Schaltkreisfamilien oder Formeln
einer relationalen Logik beschrieben werden können.

Da eine Schaltkreisfamilie eine unendliche Sequenz von Schaltkreisen
bildet, wird gewöhnlich gefordert, dass diese Schaltkreise effizient
für jede Eingabegröße $n\in\mathbb{N}$ berechnet werden können. Sind
sie in Polynomialzeit $P$ berechenbar, dann nennen wir sie $P$-uniform.

Weil boolesche Schaltkreise von Natur aus auf geordneten Strukturen
arbeiten, wird außerdem gefordert, dass ihr Ergebnis invariant bezüglich
der gewählten Ordnung ist. Wir führen eine strukturelle Einschränkung
der Schaltkreise ein, die diese Ordnungsinvarianz garantiert.
...
Ordnungsprädikat $\mathrm{LFP}+\mathbf{ORD}$. Das Ergebnis wurde
2014 von Matthew Anderson und Anuj Dawar\cite{anderson_et_al:LIPIcs:2014:4445}
veröffentlicht, und bildet die Grundlage dieser Arbeit.

Die logische Zählerweiterung $\mathrm{LFP}+C$ charakterisiert zusätzlich
die symmetrischen Schaltkreisfamilien mit einem Majority-Operator,
und die Erweiterung der Logik um beliebige numerische Prädikate opfert
nur die $P$-Uniformität.
\begin{thm}
...
äquivalent:
\begin{enumerate}
\item Die Logik $\mathrm{LFP}+\mathbf{ORD}$ und $P$-uniforme $\mathrm{SBC}$-Schaltkreisfamilien.
\item Die Logik $\mathrm{LFP}+C$ und $P$-uniforme $\mathrm{SBC}+\mathbf{MAJ}$-Schaltkreisfamilien.
\item Die Logik $\mathrm{LFP}+\mathbf{ARB}$ und $P/\mathrm{poly}$-uniforme
$\mathrm{SBC}$-Schaltkreisfamilien.
\end{enumerate}
\end{thm}
...
\end{enumerate}
\end{thm}
Die Einschränkung auf symmetrische Schaltkreisfamilien bringt eine
echte Reduktion der Ausdrucksstärke mit sich. Um dies für $\mathrm{AC}^{0}$
verwenden wir die obige Charakterisierung und ein Resultat, dass die
Logik $\mathrm{FO}+\mathbf{ARB}$ mit disjunktem Orakel von der arb-invarianten
Logik mit nicht-disjunktem Orakel trennt.
\begin{thm}
$\mathrm{FO}+\mathbf{ARB}$, invariantes $\mathrm{FO}\left(\mathrm{arb}\right)$
\begin{enumerate}
...
definierbar ist, aber nicht $\left(\mathrm{FO}+\mathbf{ARB}\right)\left[\sigma\right]$-definierbar
ist.
\end{enumerate}
\end{thm}
...
ab.
\begin{defn}
\textbf{\label{def:Tupel}Menge und Tupel}

Mengen benennen wir im allgemeinen durch gewöhnliche Großbuchstaben
wie $A$, $B$, $U$ oder $X$. Für eine Menge $A$ bezeichne $2^{A}\coloneqq\left\{ A'\mid A'\subseteq A\right\} $
...
Die Menge aller Tupel einer bestimmten Stelligkeit $k$ sei $A^{k}$.
...
\end{defn}
Die Operatoren $\min$ und $\max$ definieren das kleinste und größte
Element einer Menge unter einer bestimmten Ordnung.
\begin{defn}
...
die Verkettung, Vereinigung und Reduktion.
\begin{defn}
\textbf{Abbildung}
...
schreiben wir gegebenenfalls extensional durch auf. Für ein Tupel
...
die Identitätsfunktion auf einer festen Menge $X$.

Die Menge $\mathrm{Abb}\left(A,B\right)$ bezeichne für $\left|A\right|=\left|B\right|$
alle $n!$ bijektiven Abbildungen $\pi:A\rightarrow B$.
\end{defn}

\begin{defn}
\textbf{Permutation}

Eine Permutation von $U$ ist eine bijektive Abbildung $\pi\in\mathrm{Abb}\left(U,U\right)$.
Die Menge aller Permutationen $\mathrm{Abb}\left(U,U\right)$ bezeichnen
...
über einer Signatur $\sigma$ besteht aus einem endlichen, nicht-leeren
Universum $A$ und einer Interpretation $R^{\mathfrak{A}}\subseteq A^{k}$
für jedes Symbol $R/k\in\sigma$.
\begin{itemize}
...
sei eine bijektive Abbildung $\pi\in\mathrm{Abb}\left(A,B\right)$
ein Isomorphismus, falls $\pi R^{\mathfrak{A}}=R^{\mathfrak{B}}$
für alle Symbole $R\in\sigma$ gilt.
...
Die Menge aller Isomorphismen bezeichnen wir mit $\mathrm{Abb}\left(\mathfrak{A},\mathfrak{B}\right)$.
Zwei Strukturen heißen isomorph (kurz $\mathfrak{A}\cong\mathfrak{B}$),
falls $\mathrm{Abb}\left(\mathfrak{A},\mathfrak{B}\right)$ nicht
leer ist.

Ein Automorphismus $\pi\in\mathrm{Abb}\left(\mathfrak{A},\mathfrak{A}\right)$
sei ein Isomorphismus von $\mathfrak{A}$ zu sich selbst. Die Menge
$\mathrm{Abb}\left(\mathfrak{A},\mathfrak{A}\right)$ nennen wir $\mathrm{Aut}_{\mathfrak{A}}$;
diese bilden (genau wie die Permutationen) eine Gruppe bezüglich der
Verkettung $\circ$ und dem neutralen Element $\mathbf{id}_{\mid A}$.
\end{defn}
...
auf eine Relation $q\left(\mathfrak{A}\right)\subseteq A^{k}$.
...
und $\pi\in\mathrm{Abb}\left(\mathfrak{A},\mathfrak{B}\right)$ gilt
...
und einer Auswertungsfunktion $\left\llbracket \cdot\right\rrbracket \left(\mathfrak{A},\beta\right)$
für jede Struktur $\mathfrak{A}$ und Belegung $\beta$.
...
das Tupel $\bar{x}=x_{1}\cdots x_{k}$ ein \textbf{Argument }von $\varphi$.
Verschiedene Argumente von $\varphi$ unterscheiden sich nur in der
Reihenfolge der Variablen. Durch die Notation $\varphi\left(\bar{x}\right)$
...
ab. Im folgenden werden wir außerdem Formeln ohne Implikations-Pfeile
betrachten.
\begin{defn}
...
$\left\Vert \varphi\right\Vert $ parametrisierte Komplexitätsklassen
betrachten.
...
\end{defn}
Wir möchten syntaktisch garantieren, dass der Fixpunkt der selbstbezüglichen
Formel monoton wächst. Dafür wird verlangt, dass der Selbstbezug nicht
negiert vorkommt.
\begin{defn}
\textbf{Positivität}
...
wie folgt:
\begin{itemize}
\item Jede $\mathcal{L}\left[\sigma\right]$-Formel $\varphi$ mit $X\notin\mathrm{frei}\left(\varphi\right)$
...
\end{itemize}
\end{defn}
Nun definieren wir die Fixpunkt-Erweiterung einer Logik.
\begin{defn}
...
ist $X\bar{x}$ eine $X$-positive $\left(\mathcal{L}+\mathrm{LFP}\right)\left[\sigma\right]$-Formel.\foreignlanguage{english}{
\begin{eqnarray*}
...
$\bar{y}$ von $X$-positiven $\mathrm{\left(\mathrm{FO}+\mathrm{LFP}\right)}\left[\sigma\right]$-Termen
...
Die nicht durch den Operator gebundenen Variablen $\mathrm{frei}\left(\psi\right)\backslash\left\{ x_{1},\cdots,x_{k}\right\} \subseteq\mathrm{frei}\left(\varphi\right)$
heißen \textbf{Parameter} der Fixpunkt-Operation.

Für $X/k\in\mathbf{var}_{2}$, eine $\sigma$-Struktur $\mathfrak{A}$,
eine $X$-positive $\left(\mathrm{FO}+\mathrm{LFP}\right)\left[\sigma\right]$-Formel,
und eine Parameter-Belegung 
\[
\beta:\mathrm{frei}\left(\psi\right)\backslash\left\{ X,x_{1},\cdots,x_{k}\right\} \rightarrow A
\]
sei $F_{\beta}$ die folgende Abbildung: 
\begin{eqnarray*}
...
$\varphi^{\beta\cup\left(X\mapsto Y\right)}$ auf $\mathfrak{A}$.
...
nun, ob die Belegung des Tupels $\bar{y}$ im Fixpunkt $F_{\beta'}^{\infty}$
liegt, wobei $\beta'$ die Reduktion von $\beta$ auf die Parameter
ist: 
\begin{eqnarray*}
...
als $\mathrm{LFP}$ bezeichnet. Ferner werden wir uns auf das \emph{parameterfreie}
...
\section{Logiken mit disjunkten numerischen Erweiterungen}
...
enthält. Sei $\mathbf{ARB}:\mathbb{N}\rightarrow\mathbf{FIN}_{<}^{0}\left(\eta_{\mathrm{arb}}\right)$
...
\begin{defn}
Die Logik $\mathcal{L}+\mathbf{ORD}+C$ bezeichnen wir kurz als $\mathcal{L}+C$.\end{defn}
\begin{example}
...
ungeordneten Strukturen definierbar\cite{Ebbinghaus,Libkin2012}:\end{example}
\begin{example*}
\[
...
Die unendliche Signatur $\eta_{\mathrm{arb}}$ und die Struktur $\mathcal{N}_{\mathrm{arb}}$
sei wie in Definition \ref{def:ord} definiert.

Für eine beliebige von $\eta_{\mathrm{arb}}$ disjunkte relationale
Signatur $\sigma$ sei $\mathrm{FO}\left[\sigma\uplus\eta_{\mathrm{arb}}\right]$
...
\begin{enumerate}
\item Eine $k$-stellige $\mathrm{FO}\left[\sigma\uplus\eta_{\mathrm{arb}}\right]$-Formel
...
\item Eine $\mathrm{FO}\left[\sigma\uplus\eta_{\mathrm{arb}}\right]$-Formel
$\varphi$ sei arb-invariant, wenn sie auf allen Strukturen $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$
...
arb-invarianten $\mathrm{FO}\left[\sigma\uplus\eta_{\mathrm{arb}}\right]$-Formeln.
\end{enumerate}
\end{defn}
\begin{rem*}
...
in $\mathrm{FO}\left[\sigma\uplus\eta_{\mathrm{arb}}\right]$.\cite{EFT,Libkin2012,Schweikardt13ashort}
\end{rem*}
...
Eine boolesche Basis $\mathbb{B}$ besteht aus beliebigen Relationen
$\phi'\subseteq\left\{ 0^{i}1^{j}\mid i,j\in\mathbb{N}\right\} $.
...
$\rho:\mathcal{C}\tilde{\rightarrow}\mathcal{C}$ mit $\rho_{\mid U}=\pi$
nennen wir $\rho$ von $\pi$ \textbf{induziert}. (Nach den Definitionen
\ref{def:formal} und \ref{def:Isomorphismus} ist $\rho\in\mathrm{Sym}_{U\uplus G}$
...
\end{enumerate}
Der Schaltkreis heiße \textbf{symmetrisch}, genau dann wenn jede Permutation
...
\begin{lyxcode}
Existiert~kein~Gate~$g'<g$~mit~Equiv($g,g'$)?
\begin{lyxcode}
Rigid($g$).
\end{lyxcode}
\end{lyxcode}
...
\end{lyxcode}
Für~alle~Vorgänger~$h$~von~$g$:
\begin{lyxcode}
Falls~kein~Vorgänger~$h'<h$~mit~Equiv($h,h'$)~existiert:
\begin{lyxcode}
Finde~das~erste~Gate~$h'\in G$~mit~Equiv($h,h'$).

$i\leftarrow1$.

Für~alle~Vorgänger~$h''>h$~mit~Equiv($h,h'')$.
\begin{lyxcode}
$i\leftarrow i+1$.
\end{lyxcode}
...
$\sim^{*}$ die gleiche Anzahl Vorgänger besitzen.

Der beschriebene Algorithmus wird $\mathtt{Rigid}\left(g\right)$
nur aufrufen, wenn $g$ das erste Gate (in der Ordnung von $G$) seiner
Äquivalenzklasse bezüglich $\sim^{*}$ ist. $\mathtt{Rigid}\left(g\right)$
gibt $g$ mit seinen Markierungen aus, sucht dann für jeden Vorgänger
das erste Gate seiner Äquivalenzklasse, und zählt die dazu äquivalenten
Vorgänger.

Da die Funktionen $\mathtt{Rigid}$ und $\mathtt{Equiv}$ jeweils
nur konstant viele Variablen der Größe $\mathcal{O}\left(\log n\right)$
...
\begin{lem}
\textbf{Anderson, Melkebeek, Schweikardt und Segoufin (2012)}\cite{AMSS-locality}

Die Klasse $\mathrm{AC}^{0}$ ist äquivalent zu der Klasse der durch
arb-invariante $\mathrm{FO}\left(\mathrm{arb}\right)$-Logik definierbaren
Anfragen.
\end{lem}
Der schwere Teil des Beweises (hier nicht ausgeführt) ist die Konstruktion
...
$\forall$ für $\mathtt{AND}$), und berechnet so die Auswertung.

Aus dem Lemma lesen wir zusätzlich ab, dass $\mathrm{FO}+\mathbf{ARB}$
in der arb-invarianten $\mathrm{FO}\left(\mathrm{arb}\right)$-Logik
enthalten ist: Eine $\left(\mathrm{FO}+\mathbf{ARB}\right)$-Formel
ist durch eine symmetrische $\mathrm{AC}^{0}$-Schaltkreisfamilie
berechenbar, und die symmetrischen $\mathrm{AC}^{0}$-Schaltkreise
sind in $\mathrm{AC}^{0}$ enthalten.


\begin{lem}
Es existiert eine Graph-Anfrage $q$, die durch eine arb-invariante
$\mathrm{FO}\left(\mathrm{arb}\right)$-Formel beschrieben wird, jedoch
nicht durch $\mathrm{FO}+\mathbf{ARB}$ definierbar ist.\end{lem}
\begin{proof}
Es sei 
\end{proof}
