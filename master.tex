...
\end{enumerate}
\end{thm}
Vermutlich gelten ähnliche Bezüge auch zwischen den symmetrischen
und nicht-symmetrischen Teilen der übrigen Schaltkreisklassen.
...
Die Menge $\mathrm{Abb}\left(A,B\right)$ bezeichne für endliche $\left|A\right|=\left|B\right|$
...
der Verkettung $\circ$ und dem neutralen Element $\mathbf{id}_{\mid A}$.
\end{defn}
...
die Selbstreferenz nicht negiert vorkommt.
\begin{defn}
\textbf{Positivität}
...
\left\llbracket X\bar{x}\right\rrbracket \left(\mathfrak{A},\beta\right) & \coloneqq & \left[\beta X\right]\left(\beta\bar{x}\right)\\
 &  & \mathrm{mit}\,\beta:\mathrm{frei}\left(X\bar{x}\right)\rightarrow\bigcup_{k\in\mathbb{N}}2^{A^{k}}
\end{eqnarray*}
