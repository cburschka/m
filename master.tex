...
Das Ergebnis wurde 2014 von Matthew Anderson und Anuj Dawar veröffentlicht,
...
eine Reduktion der Ausdrucksstärke mit sich. Insbesondere wird (nach
Matthew Anderson, Dieter van Melkebeek, Nicole Schweikardt und Luc
Segoufin, 2012\cite{AMSS-locality}) $\mathrm{AC}^{0}$ ohne Symmetrie
durch die arb-invariante Logik mit nicht-disjunktem Ordnungsprädikat
charakterisiert. Diese Logik ist jedoch echt ausdrucksstärker als
die disjunkte Variante $\mathrm{FO}+\mathbf{ARB}$.
\begin{thm}
\label{thm:fo-arb}$\mathrm{FO}+\mathbf{ARB}$, invariantes $\mathrm{FO}\left(\mathrm{arb}\right)$
\begin{enumerate}
...
\end{example*}
\begin{defn}
\textbf{Zählquantor }$\exists^{=}$ (wie \cite{Schweikardt:2005:AFL:1071596.1071602},
...
\begin{lem}
\textbf{\label{lem:Anderson,-Melkebeek,-Schweikardt}Anderson, Melkebeek,
Schweikardt und Segoufin (2012)}\cite{AMSS-locality}
...
ist. Per Lemma \ref{lem:Anderson,-Melkebeek,-Schweikardt} ist diese
Anfrage in $\mathrm{AC}^{0}$, aber per Theorem \ref{thm:fo} nicht
im symmetrischen Teil von $\mathrm{AC}^{0}$ beschreibbar.
\begin{lem}
