...
beziehungsweise $f_{\Vert B'}=f'_{\Vert B'}$ gilt.\end{defn}
\begin{prop}
...
\[
\mathrm{EV}\left(g\right)\coloneqq\left\{ \pi'_{\Vert\mathrm{sp}\left(g\right)}\mid\pi'\in\mathrm{Abb}\left(A,U\right),\,\mathcal{C}\left[\pi'\mathfrak{A}\right]\left(g\right)=1\right\} 
\]


Die Menge $\mathrm{EV}\left(g\right)$ kann rekursiv konstruiert werden:
\begin{claim}
(nach Behauptung 28 aus \cite{anderson_et_al:LIPIcs:2014:4445})

Sei $\pi:A\rightarrow U$ eine Bijektion. Sei $\Pi_{\pi}\left(h\right)$
die Menge der unterschiedlichen $\mathrm{sp}\left(h\right)$-Bildreduktionen
von Bijektionen, die auf $\mathrm{sp}\left(g\right)$ bildkonsistent
zu $\pi$ sind: 
\[
\Pi_{\pi}\left(h\right)\coloneqq\left\{ \pi'_{\Vert\mathrm{sp}\left(h\right)}\mid\pi':A\rightarrow U,\,\pi'^{-1}\sim_{\mathrm{sp}\left(g\right)}\pi^{-1}\right\} 
\]


So ist das Gesamtgewicht der unter $\pi\mathfrak{A}$ erfüllten Gates
$h\in H$ gleich der gewichteten Summe von $\frac{\left|\Pi\left(h\right)\cap\mathrm{EV}\left(h\right)\right|}{\left|\Pi\left(h\right)\right|}$,
dem Anteil der erfüllenden Bijektionen in $\Pi_{\pi}\left(h\right)$.

\begin{equation}
\sum_{h\in H}\mathcal{W}\left(h,g\right)\mathcal{C}\left[\pi\mathfrak{A}\right]\left(h\right)=\sum_{h\in H}\mathcal{W}\left(h,g\right)\frac{\left|\Pi_{\pi}\left(h\right)\cap\mathrm{EV}\left(h\right)\right|}{\left|\Pi_{\pi}\left(h\right)\right|}\label{eq:behauptung-28}
\end{equation}
\end{claim}
\begin{proof}
Sei $M_{\pi}$ die Menge der zu $\pi$ in $\mathrm{sp}\left(g\right)$
bildkonsistenten Bijektionen:
\[
M_{\pi}\coloneqq\left\{ \pi'\in\mathrm{Abb}\left(A,U\right)\mid\pi'^{-1}\sim_{\mathrm{sp}\left(g\right)}\pi^{-1}\right\} 
\]


Nach Gleichung \ref{eq:cons-2} ist das Gesamtgewicht der unter $\pi'\mathfrak{A}$
erfüllten Gates $h\in G$ für alle $\pi'\in M_{\pi}$ gleich, und
daher gilt: 
\begin{eqnarray*}
\left|M_{\pi}\right|\sum_{h\in H}\mathcal{W}\left(h,g\right)\mathcal{C}\left[\pi\mathfrak{A}\right]\left(h\right) & = & \sum_{h\in H}\mathcal{W}\left(h,g\right)\sum_{\pi'\in M_{\pi}}\mathcal{C}\left[\pi'\mathfrak{A}\right]\left(h\right)
\end{eqnarray*}


Weiterhin können wir für jedes Gate $h\in H$ die Menge $M_{\pi}$
in Äquivalenzklassen bezüglich $\rho\in\Pi_{\pi}\left(h\right)$ partitionieren:
\[
=\sum_{h\in H}\mathcal{W}\left(h,g\right)\sum_{\rho\in\Pi_{\pi}\left(h\right)}\sum_{\begin{subarray}{c}
\pi'\in M_{\pi}\\
\pi'_{\Vert\mathrm{sp}\left(h\right)}=\rho
\end{subarray}}\mathcal{C}\left[\pi'\right]\left(h\right)
\]


Nach der Definition der Menge $\mathrm{EV}\left(h\right)$ gilt $\mathcal{C}\left[\pi'\right]\left(h\right)=\left[\mathrm{EV}\left(h\right)\right]\left(\rho\right)$
für alle $\pi'$ mit $\rho=\pi'_{\Vert\mathrm{sp}\left(h\right)}$:
\begin{eqnarray*}
 & = & \sum_{h\in H}\mathcal{W}\left(h,g\right)\sum_{\rho\in\Pi_{\pi}\left(h\right)}\sum_{\begin{subarray}{c}
\pi'\in M_{\pi}\\
\pi'_{\Vert\mathrm{sp}\left(h\right)}=\rho
\end{subarray}}\left[\mathrm{EV}\left(h\right)\right]\left(\rho\right)\\
 & = & \sum_{h\in H}\mathcal{W}\left(h,g\right)\sum_{\rho\in\Pi_{\pi}\left(h\right)}\left[\mathrm{EV}\left(h\right)\right]\left(\rho\right)\left|\left\{ \pi'\in M_{\pi},\,\pi'_{\Vert\mathrm{sp}\left(h\right)}=\rho\right\} \right|
\end{eqnarray*}


Weil die Partitionierung $M_{\pi}=\biguplus_{\rho\in\Pi_{\pi}\left(h\right)}\left\{ \pi'\in M_{\pi},\,\pi'_{\Vert\mathrm{sp}\left(h\right)}=\rho\right\} $
$M_{\pi}$ in isomorphe Klassen teilt, gilt $\left|\left\{ \pi'\in M_{\pi},\,\pi'_{\Vert\mathrm{sp}\left(h\right)}=\rho\right\} \right|=\frac{\left|M_{\pi}\right|}{\left|\Pi_{\pi}\left(h\right)\right|}$
für $\rho\in\Pi_{\pi}\left(h\right)$.

\begin{eqnarray*}
 & = & \sum_{h\in H}\mathcal{W}\left(h,g\right)\sum_{\rho\in\Pi_{\pi}\left(h\right)}\left[\mathrm{EV}\left(h\right)\right]\left(\rho\right)\frac{\left|M_{\pi}\right|}{\left|\Pi_{\pi}\left(h\right)\right|}\\
 & = & \sum_{h\in H}\mathcal{W}\left(h,g\right)\frac{\left|\mathrm{EV}\left(h\right)\cap\Pi_{\pi}\left(h\right)\right|\left|M_{\pi}\right|}{\left|\Pi_{\pi}\left(h\right)\right|}
\end{eqnarray*}


Durch das Kürzen von $\left|M_{\pi}\right|$ entsteht die Gleichung
\ref{eq:behauptung-28}.
\end{proof}
Sei nach Definition \ref{def:Tupel} $\bar{\mathrm{sp}}\left(g\right)$
das geordnete Tupel der Elemente von $\mathrm{sp}\left(g\right)$,
und sei $\bar{\mathrm{EV}}\left(g\right)\subseteq A^{k}$ die Relation
der $k$-Tupel $\bar{a}$, die von den Bijektionen in $\mathrm{EV}\left(g\right)$
auf $\bar{\mathrm{sp}}\left(g\right)$ abgebildet werden: 
\[
\bar{\mathrm{EV}}\left(g\right)\coloneqq\left\{ \pi^{-1}\bar{\mathrm{sp}}\left(g\right)\in A^{k}\mid\pi\in\mathrm{EV}\left(g\right)\right\} 
\]


Wir werden nun für jedes Gate $g\in G$ mit den Vorgängern $H\subseteq G$
die Menge $\mathrm{EV}\left(g\right)$ rekursiv über $\left(\mathrm{EV}\left(h\right)\right)_{h\in H}$
...
\[
\mathrm{EV}\left(g\right)\coloneqq\begin{cases}
...
und $R/k\in\sigma$ ist, dann sei $\rho_{\bar{a}}:\mathrm{sp}\left(g\right)\rightarrow A$
durch die Abbildung $\rho_{\bar{a}}=\left(\bar{\mathrm{sp}}\left(g\right)\mapsto\bar{a}\right)$
definiert:
\[
\mathrm{EV}\left(g\right)\coloneqq R^{\mathfrak{A}}\cap\left\{ \rho_{\bar{a}}\bar{t}\mid\bar{a}\in A^{\left|\mathrm{sp}\left(g\right)\right|}\right\} 
\]

\item Falls $\Sigma\left(g\right)=\wedge$ ist, 
\end{casenv}
