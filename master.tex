...
$\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$ nur aus Formeln, in
...
$\mathfrak{A}$, eine $\mathrm{FO}+\mathrm{LFP}\left[\sigma\cup\mathcal{R}\right]$-Formel
und eine Belegung $\beta:\mathrm{frei}\left(\varphi\right)\backslash\bar{x}\rightarrow A$
definieren wir eine Sequenz von $\left(\sigma\cup\mathcal{R}\right)$-Strukturen
$\left(\mathfrak{A}^{\left(i\right)}\right)_{i\in\mathbb{N}}$ wie
folgt: 
\begin{eqnarray*}
\mathfrak{A}^{\left(i\right)} & \coloneqq & \left(A,\left(R^{\mathfrak{A}}\right)_{R}R^{\left(i\right)}\right)\\
R^{\left(0\right)} & \coloneqq & \emptyset\\
R^{\left(i+1\right)} & \coloneqq & \varphi^{\beta}\left(\mathfrak{A}^{\left(i\right)}\right)
\end{eqnarray*}
Das heißt: $R^{\left(0\right)}$ ist leer, und $R^{\left(i+1\right)}$
ist das Anfrageergebnis, das unter der partiellen Belegung $\beta_{\backslash\bar{x}}$
entsteht, wenn das Symbol $R$ durch die Relation $R^{\left(i\right)}$
interpretiert wird.
