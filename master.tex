...
\section{Fixpunktlogiken und Schaltkreise}
...
\begin{thm}
\textbf{\label{thm:fp-ord}$\mathrm{FP}+\mathbf{ORD}$, $P$-uniforme
$\mathrm{SBC}$}

Die folgenden Aussagen sind für alle $\sigma$-Anfragen äquivalent:
\begin{enumerate}
...
mit Majority-Gates.
\begin{thm}
\textbf{\label{thm:fp-c}$\mathrm{FP}+C$, $P$-uniformes $\mathrm{SBC}+\mathbf{MAJ}$}
...
\begin{thm}
\textbf{\label{thm:fp-u}$\mathrm{FP}+\Upsilon$, $\mathrm{poly\left(n\right)}$-großes
$\mathrm{SBC}$}
...
\begin{thm}
\textbf{\label{thm:fp-u-c}$\mathrm{FP}+\Upsilon+C$, $\mathrm{poly}\left(n\right)$-großes
...
\section{Logik erster Stufe und Schaltkreise konstanter Tiefe}
\begin{thm}
\label{thm:5.5:fo-ord}$\mathrm{FO}+\mathbf{ORD}$, $P$-uniformes
symmetrisches $\mathrm{AC}^{0}$.\cite{IMMERMAN198686}
...
\begin{thm}
\label{thm:fo-c}$\mathrm{FO}+C$, symmetrisches $\mathrm{AC}^{0}+\mathbf{MAJ}$.
...
\begin{thm}
\label{thm:fo+u}$\mathrm{FO}+\Upsilon$, symmetrisches $\mathrm{AC}^{0}$.
...
\section{Logik erster Stufe mit disjunktem und nicht-disjunktem Orakel}
\begin{thm}
...
$\mathscr{L}\left[\sigma\right]$-Formel $\varphi\left(\bar{x}\right)$
...
für $Qx_{1}\cdots Qx_{k}\varphi$ und $\bigwedge_{i=1}^{k}\left(x_{i}=y_{i}\right)$.
...
\chapter{Von Formeln zu Schaltkreisen}
...
berechnet wird.
\end{lem}
(Dieses Lemma bildet die erste Hälfte von Theorem \ref{thm:fo-c}.)
\begin{proof}
...
$5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}$-Tiefe
berechnet wird.
\end{lem}
(Dieses Lemma bildet die erste Hälfte von \ref{thm:fo+u}.)
\begin{proof}
...
\section{Beliebige Orakel mit Zählquantor}
...
$\bar{\mathcal{C}^{\varphi}}$ mit $n^{k}\left(3\left\Vert \varphi\right\Vert +1\right)$-Tiefe
und $\left(5n^{k+\mathtt{MF}\left(\varphi\right)}+1\right)$-Größe,
...
Zur Übersicht: $\bar{x}\in\mathbf{var}^{k}$ und $\bar{z}\in\mathbf{var}^{m}$
sind die Argumente (streng geordnete Variablentupel) von $\varphi$
und $\psi$, und $\bar{y}\in\mathbf{var}^{m}$ ist ein Termtupel (mit
eventuellen Wiederholungen). Weil $\varphi$ parameter-frei ist, enthält
$\bar{x}$ genau die gleichen Variablen wie $\bar{y}$: 
\begin{eqnarray*}
\left\{ x_{1},\cdots,x_{k}\right\}  & = & \left\{ y_{1},\cdots,y_{m}\right\} \\
k & \leqslant & m
\end{eqnarray*}
...
\item [{Größe:}] Es wird induktiv bewiesen, dass $T\left(\mathcal{D}^{i}\right)\leqslant\left(3\left\Vert \psi\right\Vert +1\right)i$,
und $\left|\mathcal{D}^{i}\right|\leqslant i\cdot5n^{\mathtt{MF}\left(\psi\right)}+1$
\begin{eqnarray*}
...
T\left(\mathcal{D}^{i+1}\right) & = & T\left(\mathcal{D}^{i}\right)+T\left(\mathcal{C}_{n}^{\psi}\right)+1\\
 ...
 & \overset{\mathrm{Ann.}}{=} & 3i\left\Vert \psi\right\Vert +i+3\left\Vert \psi\right\Vert +1=\left(3\left\Vert \varphi\right\Vert +1\right)i
\end{eqnarray*}
\begin{eqnarray*}
\left|\mathcal{D}^{0}\right| & = & 1\\
\left|\mathcal{D}^{i+1}\right| & = & \left|\mathcal{D}^{i}\right|+\left|\mathcal{C}_{n}^{\psi}\right|\\
 & \leqslant & \left|\mathcal{D}^{i}\right|+5n^{\mathtt{MF}\left(\psi\right)}\\
 & \overset{\mathrm{Ann.}}{\leqslant} & i\cdot5n^{\mathtt{MF}\left(\psi\right)}+1+5n^{\mathtt{MF}\left(\psi\right)}=\left(i+1\right)\cdot5n^{\mathtt{MF}\left(\psi\right)}+1
\end{eqnarray*}
Damit gilt $T\left(\mathcal{C}_{n}^{\varphi}\right)=T\left(\mathcal{D}^{n^{k}}\right)\leqslant n^{k}\left(3\left\Vert \varphi\right\Vert +1\right)$
und $\left|\mathcal{C}_{n}^{\varphi}\right|=\left|\mathcal{D}^{n^{k}}\right|=5n^{k+\mathtt{MF}\left(\varphi\right)}+1$.
\end{description}
\end{proof}
\pagebreak{}


\section{Erweiterungen der Fixpunktlogik}

Die disjunkte Ordnungserweiterung der Fixpunktlogik nach Definition
\ref{def:fo-ord} ist immer noch durch $P/\mathrm{poly}$-uniforme
(aber $\mathrm{poly}\left(n\right)$-Tiefe) Schaltkreise entscheidbar:
\begin{lem}
\label{lem:lfp-ord}Für jede parameter-freie $\left(\mathrm{LFP}+\mathbf{ORD}\right)\left[\sigma\right]$-Formel
$\varphi\left(\bar{x}\right)$ existiert eine $\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreisfamilie
$\bar{\mathcal{C}^{\varphi}}$ mit $T\left(n\right)$-Tiefe und $S\left(n\right)$-Größe,
so dass $q_{\bar{\mathcal{C}^{\varphi}}}\left(\mathfrak{A}\right)=q_{\varphi\left(\bar{x}\right)}\left(\mathfrak{A}\right)$
für alle $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$ gilt.
\begin{eqnarray*}
T\left(n\right) & = & \left(2n+1\right)^{k}\left(3\left\Vert \varphi\right\Vert +1\right)\\
S\left(n\right) & = & 5\left(2n+1\right)^{k+\mathtt{MF}\left(\varphi\right)}
\end{eqnarray*}

\end{lem}
(Dies ist die erste Hälfte von Theorem \ref{thm:fp-ord}.)
\begin{proof}
Es wird analog zu dem Beweis von Lemma \ref{lem:lfp-circ} für jedes
$n\in\mathbb{N}$ ein $\left(\sigma\cup\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis
$\mathcal{C}_{2n+1}^{\varphi}$ über $U'=\left[1,\cdots,2n+1\right]$
konstruiert, der die Anfrage $q_{\varphi}$ auf $\mathfrak{A}\uplus\Upsilon'\left(n\right)$
für $\mathfrak{A}\in\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$
und $\Upsilon'\left(n\right)=\left(\left[n+1,2n+1\right],\leqslant\right)$
berechnet.

Dieser wird analog zu dem Beweis von Lemma \ref{lem:fo-ord} in einen
$\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis $\dot{\mathcal{C}_{n}^{\varphi}}$
über $U=\left[1,n\right]$ umgewandelt, in dem nicht-konstante Inputs
$R\bar{t}$ mit $R\notin\sigma$ oder $\bar{t}\notin U^{k}$ zu Konstanten
werden.\end{proof}
\begin{lem}
\label{lem:fp-c}Für jede parameter-freie $\left(\mathrm{LFP}+C\right)\left[\sigma\right]$-Formel
$\varphi\left(\bar{x}\right)$ existiert eine $P$-uniforme $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreisfamilie
$\bar{\mathcal{C}^{\varphi}}$ mit $T\left(n\right)$-Tiefe und $S\left(n\right)$-Größe,
so dass $q_{\bar{\mathcal{C}^{\varphi}}}\left(\mathfrak{A}\right)=q_{\varphi\left(\bar{x}\right)}\left(\mathfrak{A}\right)$
für alle $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$ gilt.
\begin{eqnarray*}
...
\end{lem}
(Dies ist die erste Hälfte von Theorem \ref{thm:fp-c}.)
\begin{proof}
Hier erweitern wir den Beweis von Lemma \ref{lem:lfp-circ} für $\mathrm{LFP}\left[\sigma\right]$
um den Konstruktionsschritt aus Lemma \ref{lem:fo-c}, um den $\left(\sigma\cup\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreis
$\mathcal{C}_{2n+1}^{\varphi}$ zu erzeugen.

Anschließend wird analog zu Lemma \ref{lem:lfp-ord} der $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreis
$\dot{\mathcal{C}_{n}^{\varphi}}$ erzeugt.\end{proof}
\begin{lem}
\label{lem:fp-u}Für jede parameter-freie $\left(\mathrm{LFP}+\mathbf{ARB}\right)\left[\sigma\right]$-Formel
$\varphi\left(\bar{x}\right)$ existiert eine $\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreisfamilie
$\bar{\mathcal{C}^{\varphi}}$ mit $T\left(n\right)$-Tiefe und $S\left(n\right)$-Größe,
so dass $q_{\bar{\mathcal{C}^{\varphi}}}\left(\mathfrak{A}\right)=q_{\varphi\left(\bar{x}\right)}\left(\mathfrak{A}\right)$
für alle $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$ gilt.
...
\end{lem}
(Dies ist die erste Hälfte von Theorem \ref{thm:fp-u}.)
\begin{proof}
Wie in Lemma \ref{lem:fo-u} werden einfach die Relationssymbole $R\in\eta_{\mathrm{arb}}$
mit in den $\left(\sigma\cup\eta_{\mathrm{arb}}\cup\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreisfamilie
$\mathcal{C}_{2n+1}^{\varphi}$ aufgenommen und dann durch Konstanten
$\mathbf{0}$ oder $\mathbf{1}$ ersetzt, um $\dot{\mathcal{C}_{n}}$
zu erzeugen.\end{proof}
\begin{lem}
\label{lem:fp-u-c}Für jede parameter-freie $\left(\mathrm{LFP}+\mathbf{ARB}+C\right)\left[\sigma\right]$-Formel
$\varphi\left(\bar{x}\right)$ existiert eine $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreisfamilie
$\bar{\mathcal{C}^{\varphi}}$ mit $T\left(n\right)$-Tiefe und $S\left(n\right)$-Größe,
so dass $q_{\bar{\mathcal{C}^{\varphi}}}\left(\mathfrak{A}\right)=q_{\varphi\left(\bar{x}\right)}\left(\mathfrak{A}\right)$
für alle $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$ gilt.
\begin{eqnarray*}
...
\end{lem}
(Dies ist die erste Hälfte von Theorem \ref{thm:fp-u-c}.)
\begin{proof}
Wie in Lemma \ref{lem:fo-u} werden einfach die Relationssymbole $R\in\eta_{\mathrm{arb}}$
mit in den $\left(\sigma\cup\eta_{\mathrm{arb}}\cup\left\{ \leqslant\right\} ,\left\{ \leqslant\right\} \right)$-Schaltkreisfamilie
$\mathcal{C}_{2n+1}^{\varphi}$ aufgenommen und dann durch Konstanten
$\mathbf{0}$ oder $\mathbf{1}$ ersetzt, um $\dot{\mathcal{C}_{n}}$
zu erzeugen.
\end{proof}
...
\chapter{Beschreibung von Symmetrischen Schaltkreis-Familien durch Fixpunktlogik}
\begin{itemize}
