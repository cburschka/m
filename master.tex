...
wir die Funktionsklassen $\mathcal{O}\left(f\right),\Omega\left(f\right)\subseteq\mathrm{Abb}\left(\mathbb{N},\mathbb{N}\right)$: \end{defn}
\begin{itemize}
...
Ein $\left(\sigma,\mathbb{B}\right)$-Schaltkreis $\mathcal{C}=\left(G,\left[W\right],\Sigma,\Omega,U\right)$
...
Orakeleingabe $\Upsilon\left(n\right)\in\left\{ 0,1\right\} ^{f\left(n\right)}$,
...
gilt, folgt:
\[
\left(\Omega_{\psi}\left(\pi'\bar{u}\right),g_{\pi'\bar{t}}\right)\in W_{\mathrm{maj}}
\]
...
dieser Gates nur einen Vorgänger bekommt.) 
\[
...
$\Sigma_{i+1}\left(\hat{\pi}g\right)=\Sigma_{i+1}\left(g\right)=\wedge$.
...
\chapter{Exkurs in die Gruppentheorie}
...
\end{defn}
Anderson und Dawar verwenden die beschriebenden Lemmata (\ref{lem:lemma-19}
...
hergestellt werden, ohne die Tiefe zu verändern - zum Beispiel hat
ein Majority-Gate $\Sigma\left(g\right)=\mathtt{MAJ}$ unter Umständen
mehrere konstante Vorgänger. In Anderson und Dawar 2014\cite{anderson_et_al:LIPIcs:2014:4445}
wird dafür die Tiefe des Schaltkreises beliebig vergrößert. Hier werden
wir stattdessen die Definition des Schaltkreises auf Multigraphen
erweitern:
\begin{defn}
\textbf{Multischaltkreis}

Ein $\left(\sigma,\mathbb{B}_{\mathrm{std}}\right)$-Multischaltkreis
$\mathcal{C}=\left(G,\left[W\right],\Sigma,\Omega,U\right)$ ist analog
zum $\left(\sigma,\mathbb{B}\right)$-Schaltkreis definiert, aber
$\left(G,\left[W\right]\right)$ bildet einen azyklischen Multigraphen.
(Hier schreiben wir $\left[W\right]$, um die Multimenge von einer
gewöhnlichen Relation zu unterscheiden. Günstigerweise entspricht
für eine gewöhnliche Relation $W\subseteq G^{2}$ die Funktion $\left[W\right]:G^{2}\rightarrow\left\{ 0,1\right\} $
gerade der entsprechenden Multimenge.)
\[
\left[W\right]:G\times G\rightarrow\mathbb{N}
\]


Für die Auswertung eines internen Gates gilt analog zu \ref{def:Auswertung-von-Schaltkreisen}:
\begin{eqnarray*}
j_{1} & \coloneqq & \sum_{h\in G}\left(\mathcal{C}\left[\mathfrak{A}\right]\left(h\right)\cdot\left[W\right]\left(h,g\right)\right)\\
j_{0} & \coloneqq & \sum_{h\in G}\left[W\right]\left(h,g\right)-j_{1}\\
...
\[
\left|\mathcal{C}\right|\coloneqq\left|G\right|+\sum_{g,g'\in G^{2}}\left[W\right]\left(g,g'\right)
\]
...
Multigraphen $\left(\hat{\pi}G,\hat{\pi}W\right)$ mit $\left[W\right]\left(\hat{\pi}g,\hat{\pi}g'\right)=\left[W\right]\left(g,g'\right)$
ab.
...
und $\left[W\right]\left(h,g\right)=\left[W\right]\left(h,g'\right)$
für alle $h\in G$.\end{defn}
\begin{lem}
...
\begin{proof}
Sei $\mathcal{D}=\left(G,W,\Sigma,\Omega,U\right)$ ein beliebiger
$k$-stelliger Schaltkreis. Wir erzeugen zunächst den äquivalenten
Multischaltkreis $\mathcal{C}\coloneqq\left(G,\left[W\right],\Sigma,\Omega,U\right)$,
wobei die Tiefe unverändert bleibt und die Größe zu $\left|\mathcal{C}\right|=\left|G\right|+\left|W\right|\leqslant\left|\mathcal{D}\right|+\left|\mathcal{D}\right|^{2}$
wird.

Berechne nun die Äquivalenzrelation $\sim\subseteq G\times G$, so
dass $g\sim g'$ genau dann wenn
\begin{enumerate}
\item $\Sigma\left(g\right)=\Sigma\left(g'\right)$, und
\item für alle $h\in W$ gilt $\left[W\right]\left(h,g\right)=\left[W\right]\left(h,g'\right)$.
\end{enumerate}
Wenn keine Gates $g,g'\in G$ mit $g\sim g'$ existieren, so ist der
Schaltkreis rigide, und der Algorithmus ist fertig.

Ansonsten sei $E\subseteq G$ eine Äquivalenzklasse von $\sim$ mit
mindestens zwei Gates und minimaler Tiefe $T\left(E\right)$. 

Wir berechnen die Funktion $c:G\rightarrow\left[0,\left|E\right|\right]$,
die für jedes Gate $h\in G$ die Vorgänger in $E$ zählt:
\[
c\left(h\right)\coloneqq\sum_{g'\in E}\left[W\right]\left(g',h\right)
\]


Sei $g\in E$ beliebig, und sei $f_{g,E}\left(\mathcal{C}\right)=\mathcal{C}'=\left(G',\left[W'\right],\Sigma',\Omega',U\right)$
...
\begin{enumerate}
\item Es werden die von $E$ ausgehenden Kanten $\left(E\times G\right)\cap W$
aus dem Schaltkreis entfernt. Außerdem werden die eingehenden Kanten
$W\cap\left(G\times E\backslash\left\{ g\right\} \right)$ der Gates
$E\backslash\left\{ g\right\} $ entfernt. Nun werden die folgenden
Kanten für jedes Gate $h\in G$ eingefügt:
\[
\left[W'\right]\left(h,i\right)\coloneqq\begin{cases}
c\left(i\right) & \mathrm{falls}\,h=g\\
0 & \mathrm{falls}\,h\in E\backslash\left\{ g\right\} \\
\left[W\right]\left(h,i\right) & \mathrm{sonst}
\end{cases}
\]
...
\end{enumerate}
Der Schaltkreis $\mathcal{C}'$ ist äquivalent zu dem Schaltkreis
$\mathcal{C}$ über jeder Struktur $\mathfrak{A}\in\mathbf{FIN}^{U}\left(\sigma\right)$,
was induktiv über die Tiefe (ausgehend von $g$) nachgewiesen wird:
\begin{enumerate}
...
\begin{eqnarray*}
j_{1} & \coloneqq & \sum_{i\in G'}\left[W\right]\left(i,h\right)\cdot\mathcal{C}\left[\mathfrak{A}\right]\\
j_{0} & \coloneqq & \sum_{i\in G'}\left[W\right]\left(i,h\right)-j_{1}\\
j_{1}' & \coloneqq & \sum_{i\in G'}\left[W'\right]\left(i,h\right)\cdot\mathcal{C}'\left[\mathfrak{A}\right]\\
j_{0}' & \coloneqq & \sum_{i\in G'}\left[W'\right]\left(i,h\right)-j_{1}'
\end{eqnarray*}
Aus der Definition von $\left[W'\right]$ folgt $j_{1}=j_{1}'$ und
$j_{0}=j_{0}'$, da die entfernten Vorgänger $g'\in E\backslash\left\{ g\right\} $
durch zusätzliche Kanten $\left(g,h\right)$ ersetzt wurden, und $\mathcal{C}\left[\mathfrak{A}\right]\left(g\right)=\mathcal{C}\left[\mathfrak{A}\right]\left(g'\right)$.
Daher gilt: 
\begin{eqnarray*}
\mathcal{C}'\left[\mathfrak{A}\right]\left(g\right) & \coloneqq & \left[\Sigma'\left(g\right)\right]\left(j_{0}',j_{1}'\right)\\
...
\item Für jedes Tupel $\bar{t}\in U^{k}$ mit $\Omega\left(\bar{t}\right)=g'\in E$
gilt:
\begin{eqnarray*}
...
\[
\sum_{h\in G}c\left(h\right)=\sum_{h\in G}\sum_{g'\in E}\left[W\right]\left(g',h\right)=\sum_{g',h'\in E\times G}
\]

\item [{Symmetrie:}] Wenn alle Äquivalenzklassen $\bar{E}=\left(E_{1},\cdots,E_{k}\right)$
der gleichen Tiefe reduziert werden, dann bewahrt der neue Schaltkreis
$f_{g_{1,}E_{1}}\cdots f_{g_{k},E_{k}}\left(\mathcal{C}\right)=\mathcal{C}'$
die Symmetrie von $\mathcal{C}$:


Sei $\pi\in\mathrm{Sym}_{U}$ eine beliebige Permutation, und $\hat{\pi}$
der induzierte Automorphismus. Offensichtlich muss $\hat{\pi}$ die
Äquivalenzklassen $\bar{E}$ aufeinander abbilden, da $\hat{\pi}g\sim\hat{\pi}g'$
für alle $g\sim g'$ gilt.
...
Da $\left[W\right]\left(\hat{\pi}g,\hat{\pi}g'\right)=\left[W\right]\left(g,g'\right)$,
folgt $\left[W'\right]\left(\hat{\pi}g,\hat{\pi}g'\right)$.
...
\begin{proof}
Sei $\mathcal{C}=\left(G,\left[W\right],\Sigma,\Omega,U\right)$ und
...
\left[W\right]\left(\hat{\pi}_{1}h,\hat{\pi}_{1}g\right) & = & \left[W\right]\left(\hat{\pi}_{2}h,\hat{\pi}_{2}g\right)=\left[W\right]\left(h,g\right)\\
 &  & \mathrm{f.a.}\,h\in G
\end{eqnarray*}
...
rigiden $\left(\sigma,\mathbb{B}\right)$-Multischaltkreises $\mathcal{C}=\left(G,\left[W\right],\Sigma,\Omega,U\right)$
und einer Permutation $\pi\in\mathrm{Sym}_{U}$ in $\mathrm{poly}\left(\left|\mathcal{C}\right|\right)$-Zeit
für jedes Gate $g\in G$ das Gate $\hat{\pi}g$ ausgibt, falls $\hat{\pi}$
existiert.\end{lem}
\begin{proof}
...
mit $\left[W\right]\left(h,g\right)>0$ bereits $\hat{\pi}h=h'$ ausgegeben
wurde.
\item Finde ein Gate $g'\in G$ mit $\Sigma\left(g\right)=\Sigma\left(g'\right)$
und $\left[W\right]\left(\hat{\pi}h,g'\right)=\left[W\right]\left(h,g\right)$
...
Für $B'\subseteq B$ heißen sie bildkonsistent in $B'\subseteq B$,
wenn $f^{-1}\sim_{B'}f^{-1}$ beziehungsweise $f_{\Vert B'}=f'_{\Vert B'}$
gilt.\end{defn}
\begin{prop}
\label{prop:konsistenz}Sei $\mathcal{C}=\left(G,W,\Sigma,\Omega,U\right)$
ein rigider, symmetrischer Multischaltkreis mit $n=\left|U\right|$,
und sei $g\in G$ ein beliebiges Gate mit den Vorgängern $H\coloneqq\left\{ h\in G\mid\left[W\right]\left(h,g\right)>0\right\} $.
...
\sum_{h\in H}\left[W\right]\left(h,g\right)\mathcal{C}\left[\pi_{1}\mathfrak{A}\right]\left(h\right) & = & \sum_{h\in H}\left[W\right]\left(h,g\right)\mathcal{C}\left[\pi_{2}\mathfrak{A}\right]\left(h\right)\label{eq:cons-2}
\end{eqnarray}
\end{prop}
\begin{proof}
...
\left[W\right]\left(h,g\right) & = & \left[W\right]\left(\hat{\tau},g\right)
\end{eqnarray*}
Es folgt die Gleichung \ref{eq:cons-2} für das Gewicht der mit $1$
belegten Vorgänger von $g$:
\[
\sum_{h\in H}\left[W\right]\left(h,g\right)\cdot\mathcal{C}\left[\pi_{1}\mathfrak{A}\right]\left(h\right)=\sum_{h\in H}\left[W\right]\left(h,g\right)\cdot\mathcal{C}\left[\pi_{2}\mathfrak{A}\right]\left(h\right)
\]

\end{proof}
Für jedes Gate $g$ des Schaltkreises $\mathcal{C}$ beschreiben wir
die Menge der verschiedenen Einbettungen $\pi:A\rightarrow U$, für
die $\mathcal{C}\left[\pi\mathfrak{A}\right]\left(g\right)=1$. Per
Satz \ref{prop:konsistenz} müssen nur deren Bildreduktionen auf $\mathrm{sp}\left(g\right)$
betrachtet werden.

Sei $\pi:A\rightarrow U$ eine beliebige Bijektion: 
\[
\mathrm{EV}\left(g\right)\coloneqq\left\{ \left(\tau\pi\right)_{\Vert\mathrm{sp}\left(g\right)}\mid\tau\in\mathrm{Sym}_{U},\,\mathcal{C}\left[\tau\pi\mathfrak{A}\right]\left(g\right)=1\right\} 
\]


Sei $\bar{\mathrm{sp}}\left(g\right)$ das geordnete Tupel der Elemente
von $\mathrm{sp}\left(g\right)$, und sei $\bar{\mathrm{EV}}\left(g\right)\subseteq A^{k}$
eine Relation von $k$-Tupeln $\bar{a}$, so dass die Abbildung $\bar{a}\mapsto\bar{\mathrm{sp}}\left(g\right)$
in $\mathrm{EV}\left(g\right)$ ist:
\[
\bar{\mathrm{EV}}\left(g\right)\coloneqq\left\{ \bar{a}\in A^{k}\mid\left(\bar{a}\mapsto\bar{\mathrm{sp}}\left(g\right)\right)\in\mathrm{EV}\left(g\right)\right\} 
\]
