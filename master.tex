...
Tupels $\bar{x}$ sei die Anzahl der Elemente, und ein $n$-stelliges
...
eine Funktion, die $\left[R\right]\left(\bar{a}\right)\coloneqq\begin{cases}
...
die folgende Abbildung:
\[
\pi\cup\pi'\left(x\right)\coloneqq\begin{cases}
...
\item Für $n\in\mathbb{N}_{\geqslant1}$ sei $\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$
die Menge aller $\sigma$-Strukturen über einem beliebigen Universum
der Größe $n$.
...
von $\left[a,b\right]$ interpretiert wird.
...
Arguments) bezeichne die Anzahl der frei vorkommenden Variablen: $\mathrm{ar}\left(\varphi\left(\bar{x}\right)\right)=\mathrm{ar}\left(\bar{x}\right)=\left|\mathrm{frei}\left(\varphi\right)\right|$.
...
Sei $q_{\varphi^{\beta}}=\left\{ \bar{a}\in A^{\ell}\mid\mathfrak{A}\models\varphi^{\beta\uplus\beta'}\right\} $
...
\begin{defn}
Die Fixpunktlogik $\left(\mathrm{FO}+\mathrm{LFP}\right)\left[\sigma\right]$
...
eine Belegung $\beta:\mathrm{frei}\left(\varphi\right)\backslash\left\{ x_{1},\cdots,x_{k}\right\} \rightarrow A$
...
X
\end{array}\right)\right)\\
F\left(X\right) & \coloneqq & \varphi^{\beta}\left(\mathfrak{A}\cup\mathfrak{R}_{X}\right)
\end{eqnarray*}
Das heißt: $F\left(X\right)$ ist das Anfrageergebnis von $\varphi^{\beta}$
auf $\mathfrak{A}$, wenn das Symbol $R$ durch die Relation $X$
interpretiert wird.
...
$F^{\infty}$ liegt: 
\begin{eqnarray*}
\left\llbracket \left[\mathrm{lfp}_{R,\bar{x}}\psi\right]\left(\bar{y}\right)\right\rrbracket \left(\mathfrak{A},\beta\right) & = & \left\llbracket R\bar{y}\right\rrbracket \left(\mathfrak{A}\cup\mathfrak{R}_{F^{\infty}\left(\emptyset\right)},\beta\right)
\end{eqnarray*}
...
und eine Belegung $\beta:\mathrm{frei}\left(\varphi\right)\rightarrow A\cup\left[0,n\right]$
wird $\varphi$ auf der disjunkten Vereinigung von $\mathfrak{A}$
mit der entsprechenden Orakelstruktur $\Upsilon\left(\left|A\right|\right)$
ausgewertet:
\[
...
Die unendliche Signatur $\eta_{\mathrm{arb}}=\left\{ R_{X}\mid k\in\mathbb{N},\,X\subseteq\mathbb{N}^{k}\right\} $
...
sei $\Upsilon_{\mathrm{arb}}:\mathbb{N}\rightarrow\mathbf{FIN}_{\leqslant}\left(\eta_{\mathrm{arb}}\right)$
...
bis auf einen konstanten Zuwachs von $\mathtt{MF}\left(\varphi\right)$
haben.
\begin{defn}
...
sei 
\[
\left\llbracket \#x\varphi\right\rrbracket \left(\mathfrak{A},\beta\right)\coloneqq\left|\left\{ a\in A\mid\mathfrak{A}\models\varphi^{\beta\uplus\binom{x}{a}}\right\} \right|
\]
die Anzahl der unterschiedlichen Werte $a\in A$, für die $\beta'=\beta\uplus\left(\begin{array}{c}
x\\
a
\end{array}\right)$ eine erfüllende Belegung von $\varphi$ auf $\mathfrak{A}$ ist.
...
1 & \mathrm{falls}\,\beta y=\left\{ a\in A\mid\mathfrak{A}\models\varphi^{\beta\cup\binom{x}{a}}\right\} \\
0 & \mathrm{sonst}
\end{cases}
\]
...
1 & \mathrm{falls}\,\beta y\in\left[0,n\right],\,\beta y\leqslant\left\{ a\in A\mid\mathfrak{A}\models\varphi^{\beta\cup\binom{x}{a}}\right\} \\
0 & \mathrm{sonst}
\end{cases}
\]
...
Die unendliche Signatur $\eta_{\mathrm{arb}}=\left\{ R_{X}\mid k\in\mathbb{N},\,X\in\mathcal{P}\left(\mathbb{N}^{k}\right)\right\} $
enthalte ein Symbol für jede beliebige Relation auf den natürlichen
Zahlen. Sei $\mathcal{N}_{\mathrm{arb}}$ die $\eta_{\mathrm{arb}}$-Struktur,
...
\item Eine $\mathrm{FO}\left[\sigma_{\mathrm{arb}}\right]$-Formel sei arb-invariant,
wenn sie auf allen Strukturen $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$
arb-invariant ist.
...
vom Typ $\phi$ hat den Wert $\left[\phi\right]\left(m,n\right)$,
wenn $m$ seiner Vorgänger den Wert $0$ und $n$ Vorgänger den Wert
$1$ haben. Als Beispiel seien die folgenden booleschen Junktoren
gegeben: 
\begin{eqnarray*}
...
die einzige Struktur in $\mathbf{FIN}_{\leqslant}^{\left[0,n\right]}\left(\emptyset\right)$
ist.
...
Nun betrachten wir die disjunkte Vereinigung $\mathfrak{A}'\uplus\Upsilon'\left(n\right)\in\mathbf{FIN}^{\left[1,2n+1\right]}\left(\sigma\cup\left\{ \leqslant\right\} \right)$
...
(die Isomorphie ist unter disjunkter Vereinigung abgeschlossen). Also
gilt: 
\[
...
Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert \cdot\left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}$
berechnet wird.
\end{lem}
(Dieses Lemma bildet die erste Hälfte von Theorem \ref{thm:5.6:fo-c}.)
\begin{proof}
Für eine $k$-stellige $\left(\mathrm{FO}+\exists^{\geqslant}\right)\left[\sigma\right]$-Formel
$\varphi$ konstruieren wir den $\left(\sigma\cup\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreis
$\mathcal{C}_{2n+1}^{\varphi}$ über dem Universum $U'\coloneqq\left[1,2n+1\right]=\left[1,n\right]\uplus\rho\left[0,n\right]$
nach dem gleichen Schema wie in Lemma \ref{lem:fo-ord}, wobei ein
...
\begin{description}
\item [{Schaltkreis:}] $\varphi\left(\bar{x}\right)=\exists^{\geqslant x_{i}}y_{j}\psi\left(\bar{y}\right)$,
...
erhalten:
\begin{eqnarray*}
W_{\mathrm{pad}} & = & \left\{ \left(0_{j},g_{\bar{t}}\right)\mid\bar{t}\in U'^{k},\,t_{i}\in\left[n+1,2n+1\right],\,1\leqslant j\leqslant2t_{i}-3n-2\right\} \\
 & \cup & \left\{ \left(1_{j},g_{\bar{t}}\right)\mid\bar{t}\in U'^{k},\,t_{i}\in\left[n+1,2n+1\right],\,1\leqslant j\leqslant3n+2-2t_{i}\right\} 
\end{eqnarray*}
\begin{eqnarray*}
...
denn der neue Schritt vergrößert die Tiefe nur um eine Kante. Weiterhin
hat er die Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}$,
denn der neue Schritt fügt höchstens 
\[
\left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}+2n<5\left(2n+1\right)^{\mathtt{MF}\left(\varphi\right)}
\]
neue Gates hinzu.
