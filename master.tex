...
\begin{enumerate}
\item Klassen von uniformen booleschen Schaltkreisfamilien $\left(\mathcal{C}_{n}\right)_{n\in\mathbb{N}}$
(mit $\mathtt{AND}$-, $\mathtt{OR}$-, $\mathtt{NOT}$-, und gegebenenfalls
Majority-Gates) beschreiben Anfragen, deren Datenkomplexität auf $T_{n}\cdot\mathcal{O}\left(\log S_{n}\right)$-Platz
und $S_{n}$-Zeit beschränkt ist, wobei $T_{n}$ und $S_{n}$ die
Tiefe und Größe jedes Schaltkreises sind.
\item Formeln der Prädikatenlogik erster Stufe (beziehungsweise deren Erweiterungen)
beschreiben Anfragen, deren Datenkomplexität für eine Formel $\varphi$
auf $\mathcal{O}\left(\log n\right)$-Platz und $\mathrm{poly}\left(n\right)$-Zeit
beschränkt ist.
\end{enumerate}
Boolesche Schaltkreise arbeiten per Definition aus auf einer geordneten
Struktur. Für ungeordnete Strukturen wird eine beliebige Ordnung gewählt
und gefordert, dass das Ergebnis bezüglich der Ordnung invariant ist.
Wir führen mit der Symmetrie eine strukturelle Einschränkung der Schaltkreise
...
eine solche Sequenz auch unentscheidbare Klassen in den natürlichen
Zahlen kodieren. In der Praxis ist es daher erwünscht, dass die Schaltkreise
...
sei ein \textbf{Träger} von\textbf{ }$G$ genau dann wenn $\mathrm{Stab}_{U}\left(\mathcal{P}\right)\subseteq G$.
\end{defn}
Wenn $\mathcal{P}$ ein Träger von $G$ ist, so ist $\mathcal{P}$
auch ein Träger jeder Obermenge $G'\supseteq G$. Außerdem sind per
Satz \ref{prop:feinheit-stab} alle feineren Partitionen $\mathcal{P}'\preceq\mathcal{P}$
Träger von $G$. Somit ist $\mathcal{P}_{\min}=\left\{ \left\{ u_{1}\right\} ,\cdots,\left\{ u_{n}\right\} \right\} $
...
\begin{lem}
\textbf{\label{lem:fo-arb-ac0}Makowsky (1997)}\cite{Makowsky:1997:ID:648040.744731,AMSS-locality}
