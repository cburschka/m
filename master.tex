...
äquivalent:
\begin{enumerate}
\item Die Logik $\mathrm{FP}+\mathbf{ORD}$ und $P$-uniforme $\mathrm{SBC}$-Schaltkreisfamilien.
\item Die Logik $\mathrm{FP}+C$ und $P$-uniforme $\mathrm{SBC}+\mathbf{MAJ}$-Schaltkreisfamilien.
...
$\mathrm{SBC}+\mathbf{MAJ}-$Schaltkreisfamilien.
\end{enumerate}
\end{thm}
Das Ergebnis wird für Schaltkreisfamilien konstanter Tiefe angepasst,
in dem die technischen Resultate des Artikels mit Hinblick auf die
Tiefe abgeändert werden. Diese Schaltkreisfamilien werden durch die
Logik erster Stufe ohne Fixpunktoperator charakterisiert.
\begin{thm}
Die folgenden Paare von Anfragenklassen sind auf endlichen Strukturen
äquivalent:
\begin{enumerate}
...
\end{enumerate}
\end{thm}
Schließlich werden zwei Erweiterungen der Logik erster Stufe verglichen
und ein echter Einschluss mit Hilfe der Schaltkreisfamilien nachgewiesen.
\begin{thm}
...
\section{Relationale Logik erster Stufe}
...
Die disjunkte Orakelerweiterung fügt einer Logik eine Anzahl von Relationssymbolen
hinzu, die für $\mathfrak{A}\in\mathbf{FIN}\left(\sigma\right)$ eine
feste Interpretation über einem von $A$ disjunkten numerischen Universum
erhalten.
\begin{defn}
...
eine Permutation $\pi\in\mathrm{Sym}_{U}$ und Automorphismus $\rho:\mathcal{C}\tilde{\rightarrow}\mathcal{C}$
heiße $\rho$ \textbf{induziert} von $\pi$, wenn $\rho_{\mid U}=\pi$.
(Nach den Definitionen \ref{def:formal} und \ref{def:Isomorphismus}
ist $\rho\in\mathrm{Sym}_{U\uplus G}$ eine Permutation des Universums
von $\mathcal{C}$.) Wir können aus der formalen Definition des Schaltkreises
als $\tau_{\sigma,\mathbb{B},k}$-Struktur ableiten, dass ein von
$\pi$ induzierter Automorphismus $\rho$ die folgenden Bedingungen
erfüllt:
\begin{enumerate}
...
permutiert, da $\hat{\pi}=\hat{\pi}_{\mid G}\uplus\pi$.)\end{defn}
\begin{prop}
