...
mit polynomieller Länge und einer konstanten Anzahl Variablen definiert
werden, so dass $\varphi_{n}$ auf den Strukturen der Größe $n$ äquivalent
zu $\varphi$ ist. (Vergleichbar zum $k$-Variablen-Fragment der infinitären
Logik $L_{\infty\omega}^{k}$, welches die Fixpunktlogik einschließt.\cite{Dawar1995a,Kolaitis1992a})
...
\begin{lem}
Für jede $\mathrm{LFP}\left[\sigma\right]$-Formel $\varphi$ und
jedes $n\in\mathbb{N}$ existiert eine $\mathrm{FO}\left[\sigma\right]$-Formel
$\hat{\varphi}$ mit $\left\Vert \hat{\varphi}\right\Vert \leqslant n^{\left\Vert \varphi\right\Vert }$
und $\mathtt{MF}\left(\hat{\varphi}\right)\leqslant2\mathtt{MF}\left(\varphi\right)$,
...
mit dem Argument $\bar{x}\in\mathbf{var}^{k}$.

Die Tatsache, dass jeder $\mathrm{LFP}\left[\sigma\right]$-Term $t$
eine Variable $t\in\mathbf{var}$ ist, macht die atomaren Fälle trivial:
\begin{casenv}
\item Falls $\varphi=R\bar{y}$ für ein $k$-Tupel von $\mathrm{LFP}\left[\sigma\right]$-Termen
$\bar{y}\in\mathbf{var}^{k}$ und $R/k\in\sigma$, so ist $\hat{\varphi}=\varphi$
bereits eine $\mathrm{FO}\left[\sigma\right]$-Formel.
\item Falls $\varphi=y_{1}\dot{=}y_{2}$ für zwei $\mathrm{LFP}\left[\sigma\right]$-Terme
$y_{1},y_{2}\in\mathbf{var}$, so ist $\hat{\varphi}=\varphi$ bereits
eine $\mathrm{FO}\left[\sigma\right]$-Formel.
\item Falls $\varphi=\left(\psi_{1}\gamma\cdots\gamma\psi_{k}\right)$ für
$k$ $\mathrm{LFP}\left[\sigma\right]$-Formeln $\psi_{1},\cdots,\psi_{k}$
...
$\hat{\psi_{1}},\cdots,\hat{\psi_{k}}$.
\[
\hat{\varphi}\coloneqq\left(\hat{\psi_{1}}\gamma\cdots\gamma\hat{\psi_{k}}\right)
\]

\item Falls $\varphi=\neg\psi$ für eine $\mathrm{LFP}\left[\sigma\right]$-Formel
$\psi$, so existiert per Annahme die $n$-äquivalente Formel $\hat{\psi}$:
\begin{eqnarray*}
\hat{\varphi} & \coloneqq & \neg\hat{\psi}
\end{eqnarray*}

\item Falls $\varphi=Qy\psi$ für $Q\in\left\{ \exists,\forall\right\} $,
$y\in\mathbf{var}$ und eine $\mathrm{LFP}\left[\sigma\right]$-Formel
$\psi$ , so existiert per Annahme die $n$-äquivalente Formel $\hat{\psi}$:
\begin{eqnarray*}
\hat{\varphi} & \coloneqq & Qy\hat{\psi}
\end{eqnarray*}

\item Falls $\varphi=\left[\mathrm{lfp}_{R,\bar{z}}\psi\right]\left(\bar{y}\right)$
für eine $k$-stellige Relationsvariable $R\in\mathbf{var}_{2}$,
$\bar{z}\in\mathbf{var}^{k}$, eine $\mathrm{LFP}\left[\sigma\right]$-Formel
...
\begin{eqnarray*}
\psi_{0} & \coloneqq & \neg\bar{z}\dot{=}\bar{z}\\
\psi_{i+1} & \coloneqq & \hat{\psi}^{\left(\frac{R\bar{t}}{\chi_{i}\left(\bar{t}\right)}\right)}\\
\chi_{i}\left(\bar{t}\right) & \coloneqq & \exists\bar{u}\left(\bar{t}=\bar{u}\wedge\exists\bar{z}\left(\bar{u}=\bar{z}\wedge\psi_{i}\right)\right)
\end{eqnarray*}
Hierbei sei $\hat{\psi}^{\left(\frac{R\bar{t}}{\chi_{i}\left(\bar{t}\right)}\right)}$
die Formel, die aus $\hat{\psi}$ entsteht, indem jedes Atom $R\bar{t}$
(für irgendein $\bar{t}\in\mathbf{var}^{k}$) durch $\chi_{i}\left(\bar{t}\right)$
...
Nun gelte:
\[
\hat{\varphi}\coloneqq\exists\bar{z}\left(\bar{x}=\bar{z}\wedge\psi_{n^{k}}\right)
\]
...
wie folgt: 
\[
X_{i}\coloneqq\left\{ \bar{a}\in A^{k}\mid\mathfrak{A}\models\psi_{i}\left[\bar{a}\right]\right\} 
\]
...
\begin{eqnarray*}
\bar{a}\in X_{i+1} & \Leftrightarrow & \mathfrak{A}\models\psi_{i+1}\left[\bar{a}\right]\\
 & \Leftrightarrow & \mathfrak{A}\models\hat{\psi}^{\left(\frac{R\bar{z}}{\chi_{i}\left(\bar{z}\right)}\right)}\left[\bar{a}\right]
\end{eqnarray*}
Per Definition gilt: 
\[
X_{i}\coloneqq\left\{ \bar{a}\mid\mathfrak{A}\models\psi_{i}\left[\bar{a}\right]\right\} =\left\{ \bar{a}\mid\mathfrak{A}\models\chi_{i}\left[\bar{a}\right]\right\} 
\]
\begin{eqnarray*}
...
Daher gilt:
\begin{eqnarray*}
\left\llbracket \hat{\varphi}\right\rrbracket \left(\mathfrak{A},\bar{a}\right) & = & \left\llbracket \exists\bar{u}\left(\bar{u}=\bar{y}\wedge\psi_{n^{k}}\right)\right\rrbracket \left(\mathfrak{A},\bar{a}\right)\\
