...
Für den $P$-uniformen Teil der Klasse $\mathrm{SBC}$ von symmetrischen
booleschen Schaltkreisfamilien wird eine logische Charakterisierung
durch die Fixpunktlogik mit disjunktem Ordnungsprädikat $\mathrm{LFP+\mathbf{ORD}}$
beschrieben.

Es wird gezeigt, dass die logische Zählerweiterung $\mathrm{LFP}+C$
zusätzlich die Schaltkreisfamilien mit einem Majority-Operator charakterisiert.

Schließlich wird gezeigt, dass die Erweiterung der Logik um beliebige
numerische Prädikate alle symmetrischen $P/\mathrm{poly}$-uniformen
Schaltkreisfamilien charakterisiert. (Dieses Resultat ist mit dem
für die Zählerweiterung kombinierbar.)
\begin{thm}
...
\end{enumerate}
\end{thm}
Das Ergebnis wird für Schaltkreisfamilien konstanter Tiefe angepasst,
in dem die technischen Resultate von Anderson und Dawar mit Hinblick
auf den Erhalt der Schaltkreistiefe abgeändert werden. Es wird eine
Charakterisierung der symmetrischen $\mathrm{AC}^{0}$-Schaltkreisfamilien
...
echte Reduktion der Ausdrucksstärke mit sich. Zum Nachweis für $\mathrm{AC}^{0}$
...
schreiben wir gegebenenfalls extensional auf. Für ein Tupel $\bar{a}=\left(a_{1},\cdots,a_{n}\right)$
kürzen wir diese Abbildung auch durch $\left(\bar{a}\mapsto\pi\bar{a}\right)$
ab. 
\begin{eqnarray*}
...
Für eine Struktur $\mathfrak{A}$ sei eine \textbf{Belegung} $\beta$
...
$\varphi\left(\bar{x}\right)$ gelte $\mathrm{ar}\left(\varphi\right)=\mathrm{ar}\left(\bar{x}\right)=\left|\mathrm{frei}\left(\varphi\right)\right|$.
...
Entsprechend sei $q_{\varphi\left(\bar{x}\right)}\left(\mathfrak{A}\right)\coloneqq\left\{ \bar{a}\in A^{\mathrm{ar}\left(\bar{x}\right)}\mid\mathfrak{A}\models\varphi\left[\bar{a}\right]\right\} $
...
$\bar{x}$ abhängt, wird es durch $q_{\varphi\left(\bar{x}\right)}$
mit angegeben.)
\end{defn}
...
werden wir hauptsächlich diese behandeln. (Hierbei entsteht ein fester
Zuwachs in der Länge der Formel $\left\Vert \varphi\right\Vert $,
...
$\varphi$ ist \textbf{parameter-frei}, falls der Fixpunktoperator
stets alle Variablen bindet - das heißt, für jede Teilformel der Form
$\left[\mathrm{lfp}_{R,\bar{x}}\psi\right]\left(\bar{y}\right)$ gilt:
\[
\mathrm{frei}\left(\psi\right)\subseteq\left\{ x_{1},\cdots,x_{\mathrm{ar}\left(\bar{x}\right)}\right\} 
\]
\end{defn}
\begin{prop}
...
parameter-freie $\mathrm{LFP}\left[\sigma\right]$-Formel $\varphi'$
übersetzt werden.\cite{Zaid,Grohe2005,Dziembowski96bounded-variablefixpoint}
\end{prop}
\pagebreak{}


\section{Numerische Erweiterungen}
...
\begin{defn}
\textbf{\label{def:fo-ord}$\mathcal{L}+\mathbf{ORD}$, $\mathcal{L}+\mathbf{BIT}$,
$\mathcal{L}+\mathbf{ARB}$.}
\begin{enumerate}
\item Sei $\mathbf{ORD}:\mathbb{N}\rightarrow\mathbf{FIN}_{<}^{0}\left(\emptyset\right)$
...
ausgibt.
\item Sei $\mathbf{BIT}:\mathbb{N}\rightarrow\mathbf{FIN}_{<}^{0}\left(\left\{ \mathtt{BIT}\right\} \right)$
...
\mathbf{BIT}\left(n\right) & \coloneqq & \left(\left[0,n\right],\leqslant_{\mid\left[0,n\right]},\mathtt{BIT}_{\mid\left[0,n\right]}^{\mathcal{N}}\right)\in\mathbf{FIN}_{<}^{\left[0,n\right]}\left(\left\{ \mathtt{BIT}\right\} \right)
\end{eqnarray*}

\item Sei $\eta_{\mathrm{arb}}=\left\{ R_{X}\mid X\in\bigcup_{k\in\mathbb{N}}2^{\mathbb{N}^{k}}\right\} $
...
\begin{eqnarray*}
\mathcal{N}_{\mathrm{arb}} & \coloneqq & \left(\mathbb{N},\,\left(R_{X}^{\mathcal{N}}\right)_{R_{X}\in\eta_{\mathrm{arb}}}\right)\\
R_{X}^{\mathcal{N}} & \coloneqq & X\\
\mathbf{ARB}\left(n\right) & \coloneqq & \left(\mathcal{N}_{\mathrm{arb}}\right)_{\mid\left[0,n\right]}
\end{eqnarray*}
...
sei wie in Definition \ref{def:fo-ord} definiert.
...
Der beschriebene Algorithmus wird ein Gate und seine ausgehenden Kanten
erst ausgeben, nachdem sichergestellt ist, dass es das erste Gate
(in der Ordnung von $G$) seiner Äquivalenzklasse bezüglich $\sim^{*}$
ist. Danach werden alle weiteren Gates der gleichen Klasse übersprungen,
und ihre Nachfolger (jeweils die ersten ihrer Äquivalenzklasse) stattdessen
mit $g$ verbunden.
...
wobei $T$ die maximale Rekursionstiefe von $\mathtt{Rigid}$ und
$\mathtt{Equiv}$ ist. Weil jeder Aufruf für $g$ nur Aufrufe für
Vorgänger von $g$ auslöst, ist $T=T\left(\mathcal{C}\right)$ die
Tiefe des Schaltkreises $\mathcal{C}$.
\end{proof}
\begin{algorithm}
\begin{lyxcode}
...
Gib~alle~ausgehenden~Kanten~$\left(g,h\right)$~aus.

Gib~$\Omega\left(\bar{t}\right)=g$~für~alle~$\bar{t}\in\Omega^{-1}\left(g\right)$~aus.

Für~jedes~Gate~$g'>g$:
\begin{lyxcode}
Falls~Equiv($g,g'$):
\begin{lyxcode}
Für~alle~Nachfolger~$h$~von~$g'$:
\begin{lyxcode}
Existiert~kein~Gate~$h'<h$~mit~Equiv($h,h'$)?
\begin{lyxcode}
Dann~gib~die~Kante~$\left(g,h\right)$~aus.
\end{lyxcode}
\end{lyxcode}
Für~$\bar{t}\in\Omega^{-1}\left(g'\right)$:
\begin{lyxcode}
