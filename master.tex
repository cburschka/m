...
durch die invariante Logik $\mathrm{FO}\left(\mathbf{ARB}\right)$
(Neil Immerman, 1987\cite{Immerman1987}), und $P/\mathrm{poly}$
durch $\mathrm{FP}\left(\mathbf{ARB}\right)$ (Johann Makowsky, 1997\cite{Makowsky1997-FO,Makowsky1998-LFP})
charakterisiert. $\mathrm{FO}\left(\mathbf{ARB}\right)$ ist jedoch
echt ausdrucksstärker als die disjunkte Variante $\mathrm{FO}+\mathbf{ARB}$:
\begin{thm}
...
\begin{prop}
Isomorphismen sind unter disjunkter Vereinigung abgeschlossen.\end{prop}
\begin{proof}
Seien $\mathfrak{A},\mathfrak{A}'\in\mathbf{FIN}\left(\sigma_{1}\right)$
und $\mathfrak{B},\mathfrak{B}'\in\mathbf{FIN}\left(\sigma_{2}\right)$
jeweils zwei Paare isomorphere Strukturen mit $\pi_{1}\in\mathrm{Bij}\left(\mathfrak{A},\mathfrak{A}'\right)$
und $\pi_{2}\in\mathrm{Bij}\left(\mathfrak{B},\mathfrak{B}'\right)$.
Sei ferner $A\cap B=A'\cap B'=\emptyset$.
...
und $\mathfrak{A}\uplus\mathfrak{B}\cong\mathfrak{A}'\uplus\mathfrak{B}'$.
...
\begin{defn}
\textbf{Nachbarschaft} (siehe Abschnitt 2.4 von \cite{EbbinghausFlum})
...
verbindet, die im gleichen Tupel vorkommen. (Insbesondere ist für
jeden symmetrischen Graphen $\mathfrak{A}\in\mathbf{FIN}\left(\left\{ E\right\} \right)$
...
E^{\mathcal{G}\left(\mathfrak{A}\right)} & = & \left\{ \left(a_{i},a_{j}\right)\mid R/k\in\sigma,\,\bar{a}\in R^{\mathfrak{A}},\,i,j\in\left[1,k\right]\right\} 
\end{eqnarray*}
...
im Gaifman-Graph zwischen zwei Knoten $a,b\in A$.

Für ein $r\in\mathbb{N}$ sei nun $S_{r}\left(a\right)\coloneqq\left\{ b\in A\mid\mathrm{dist}_{\mathfrak{A}}\left(a,b\right)\leqslant r\right\} $
die $r$-Kugel um $a\in A$. Die $r$-Nachbarschaft von $a$ sei deren
induzierte Teilstruktur $\mathcal{S}_{r}\left(a\right)=\mathfrak{A}_{\mid S_{r}\left(a\right)}$
in $\mathfrak{A}$.
\end{defn}
...
\begin{lem}
\label{lem:counting-circuit}Sei $\mathcal{L}$ eine Logik, $\mathbb{B}$
eine boolesche Basis und $\mathcal{K}\supseteq\mathrm{LOGSPACE}$
eine Komplexitätsklasse, so dass für jede $\mathcal{L}\left[\sigma\right]$-Formel
eine symmetrische, $\mathcal{K}$-uniforme $\left(\sigma,\mathbb{B}\right)$-Schaltkreisfamilie
mit $t\left(n\right)$-Tiefe und $\mathrm{poly}\left(n\right)$-Größe
existiert, die die gleiche Anfrage beschreibt.
...
mit der Tiefe $t'\left(n\right)\coloneqq t\left(2n+1\right)+\left\Vert \varphi\right\Vert $
und $\mathrm{poly}\left(n\right)$-Größe: 
\end{lem}
Insbesondere folgt aus der Kombination mit Lemma \ref{lem:fo-circuit}
und Lemma \ref{lem:oracle-circuit}:
...
und $n\in\mathbb{N}$ die gewünschte Eingabegröße. Wir gehen analog
zu Lemma \ref{lem:oracle-circuit} für $\mathcal{L}+\Upsilon$ vor
und erzeugen einen $\left(\sigma\uplus\eta\uplus\left\{ \leqslant\right\} ,\mathbb{B}\cup\left\{ \mathtt{MAJ}\right\} \right)$-Schaltkreis
über $U'=\left[1,2n+1\right]$, wobei wir aber die folgende induktive
Konstruktion verwenden:
\begin{casenv}
\item Falls $\varphi\left(\bar{x}\right)$ eine $\left(\mathcal{L}+\Upsilon\right)\left[\sigma\right]$-Formel
ohne $\exists^{\geqslant}$-Quantor ist, so existiert der $\left(\sigma\uplus\eta\uplus\left\{ \leqslant\right\} ,\mathbb{B}\right)$-Schaltkreisfamilie
$\mathcal{C}_{2n+1}^{\varphi}$ per Voraussetzung.
\item Falls $\varphi\left(\bar{x}\right)=\exists^{\geqslant x_{i}}y_{j}\psi\left(\bar{y}\right)$
mit $\bar{x}\in\mathbf{var}^{k}$, so sei $\mathcal{C}_{2n+1}^{\psi}$
der Schaltkreis für die Formel $\psi$ (dieser hat per Annahme die
Tiefe $t\left(2n+1\right)+\left\Vert \psi\right\Vert $ und Größe
$s\left(n\right)\in\mathrm{poly}\left(n\right)$).
\[
...
\end{eqnarray*}
Der neue Schaltkreis hat die Tiefe $t\left(2n+1\right)+\left\Vert \psi\right\Vert +1\leqslant t\left(2n+1\right)+\left\Vert \varphi\right\Vert $
und der Größe $s\left(n\right)+n^{k}+2n\in\mathrm{poly}\left(n\right)$.
...
$\mathcal{C}_{2n+1}^{\varphi'}$ wird. (Eine solche Suche im Syntax-Baum
der Formel hat $\mathrm{LOGSPACE}$-Komplexität.)


Per Annahme existiert für jede der Teilformeln $\psi_{1},\cdots,\psi_{m}$
ein Schaltkreis $\mathcal{C}_{2n+1}^{\psi_{i}}$. Wir erzeugen den
Schaltkreis $\mathcal{C}_{2n+1}^{\varphi}$ aus der disjunkten Vereinigung
wie folgt:
\begin{eqnarray*}
...
\end{cases}
\end{eqnarray*}
Es gilt $\left\Vert \varphi'\right\Vert +\max\left\Vert \psi_{i}\right\Vert +1\leqslant\left\Vert \varphi\right\Vert $
(weil $\varphi$ alle diese Teilformeln enthält). Daher ist die Tiefe
des neuen Schaltkreises $t\left(2n+1\right)+\left\Vert \varphi\right\Vert $,
und seine Größe ist in $\mathrm{poly\left(n\right)}$.
...
jedes Majority-Gates $g_{\bar{t}}$ hängt von dem Wert $t_{i}$ ab.
Allerdings ist er symmetrisch bezüglich der Permutationen, die die
Werte in $\left[n+1,2n+1\right]$ fixieren, denn für alle übrigen
Werte von $t_{i}$ ist das Gate $g_{\bar{t}}$ eine $\mathbf{0}$-Konstante. 
...
\subsection{Von Fixpunkt-Formeln zu Schaltkreisfamilien}

Im folgenden möchten wir uns gerne auf Formeln beschränken, die nur
einen einzigen Fixpunktoperator enthalten. Diese Normalform schränkt
die Allgemeinheit nicht ein, was in \cite{EbbinghausFlum} (Abschnitt
8.2) für $\mathrm{LFP}\left(\mathrm{FO}\right)$ gezeigt wird.
\begin{lem}
\label{lem:lfp-fo-normalform}(Lemma 8.2.4 aus \cite{EbbinghausFlum})

Jede $\mathrm{LFP}\left(\mathrm{FO}\right)\left[\sigma\right]$-Formel
$\varphi$ ist äquivalent zu einer $\mathrm{LFP}\left(\mathrm{FO}\right)\left[\sigma\right]$-Formel
$\hat{\varphi}$ der folgenden Form, wobei $X/k\in\mathbf{var}_{2}$,
und $\psi$ eine $\mathrm{FO}\left[\sigma\uplus\left\{ X/k\right\} \right]$-Formel
mit $\mathrm{frei}\left(\psi\right)=\left\{ x_{1},\cdots,x_{k}\right\} $
ist: 
\[
\left(Q_{i}z_{i}\right)_{\begin{subarray}{c}
i\in\left[1,m\right]\\
Q_{i}\in\left\{ \exists,\forall\right\} 
\end{subarray}}\left[\mathrm{lfp}_{X,\bar{x}}\psi\right]\left(\bar{y}\right)
\]

\end{lem}
Das Lemma für $\mathrm{LFP}\left(\mathrm{FO}\right)$ gilt automatisch
auch für jede numerische Erweiterung $\mathrm{LFP}\left(\mathrm{FO}+\Upsilon\right)$
für ein $\eta$-Orakel $\Upsilon$, denn eine $\mathrm{LFP}\left(\mathrm{FO}+\Upsilon\right)\left[\sigma\right]$-Formel
$\varphi$ gleicht syntaktisch einer $\mathrm{LFP}\left(\mathrm{FO}\right)\left[\sigma\uplus\eta\right]$-Formel
$\varphi'$. Wenn dann $\varphi'$ per Lemma \ref{lem:lfp-fo-normalform}
auf jeder endlichen $\left(\sigma\uplus\eta\right)$-Struktur äquivalent
zu einer Formel $\hat{\varphi}'$ der obigen Form ist, dann gilt diese
Äquivalenz insbesondere auch für die Auswertung von $\varphi$ auf
einer $\sigma$-Struktur $\mathfrak{A}$, die disjunkt mit der $\eta$-Struktur
$\Upsilon\left(\left|A\right|\right)$ vereinigt wird.
\begin{lem}
...
Sei $\varphi$ eine beliebige $\mathrm{LFP}\left(\mathcal{L}\right)\left[\sigma\right]$-Formel
der in Lemma \ref{lem:lfp-fo-normalform} beschriebenen Normalform:
\[
\varphi=\left(Q_{i}z_{i}\right)_{\begin{subarray}{c}
i\in\left[1,m\right]\\
Q_{i}\in\left\{ \exists,\forall\right\} 
\end{subarray}}\left[\mathrm{lfp}_{X,\bar{x}}\psi\right]\left(\bar{y}\right)
\]
...
Das Ergebnis von Anderson und Dawar 2014\cite{AD2014} beschränkt
sich auf symmetrische Schaltkreisfamilien mit polynomieller Größe
(und daher automatisch polynomiell beschränkten Orbits). Hier wird
für jedes Gate nachgewiesen, dass sein Wert nur von $\mathcal{O}\left(1\right)$
...
\]
Damit ist $\mathrm{S}\left(\mathcal{C}\right)\leqslant\frac{33}{\epsilon}\frac{\log s}{\log n}\leqslant\frac{33}{\epsilon}\frac{k\log n}{\log n}=\frac{33k}{\epsilon}\in\mathcal{O}\left(1\right)$. 
\end{proof}
...
\begin{thm}
\textbf{\label{lem:fo-arb-ac0}Neil Immerman (1987)}\cite{Immerman1987}
...
\end{thm}
Der verwendete Teil des Beweises (hier nur skizziert) ist die Konstruktion
einer Formel für eine beliebige $\mathrm{AC}^{0}$-Schaltkreisfamilie.
Dazu wird jeder Schaltkreis in eine alternierende Normalform der Tiefe
$d$ gebracht, in der jedes Gate $n^{k}$ Vorgänger hat. Die $n^{kd}$
Wege von einem Input zu einem Output werden durch numerische Tupel
kodiert (die Relation dieser Wege ist ein numerisches Prädikat und
daher in $\mathrm{inv}\left(\mathrm{FO}\oplus\mathbf{ARB}\right)$-Logik
...
Die Logik erster Stufe mit disjunktem $\mathbf{ARB}$-Orakel ist verständlicherweise
nicht stärker als die mit nicht-disjunktem Orakel.
...
ist. Per Lemma \ref{lem:fo-arb-ac0} ist diese Anfrage dann in $\mathrm{AC}^{0}$,
aber per Theorem \ref{thm:fo} nicht in $\mathrm{SAC}^{0}$ beschreibbar.

Wir werden hierfür die Sätze von Hanf und von Ehrenfeucht verwenden,
um die Nicht-Definierbarkeit nachzuweisen: 
\begin{thm}
\textbf{Der Satz von Hanf}~(2.4.1 in \cite{EbbinghausFlum})

Seien $\mathfrak{A},\mathfrak{B}\in\mathbf{FIN}\left(\sigma\right)$,
$m\in\mathbb{N}$ und $r\coloneqq3^{m}$ \end{thm}
\begin{lem}
...
der Graphen $\mathfrak{A}=\left(A,E^{\mathfrak{A}}\right)$ mit $n\coloneqq A$
für die gilt: $A$ enthält weniger als $\left\lceil \mathrm{log}n\right\rceil $
viele nicht-isolierte Knoten.

Diese Anfrage ist durch einen $\mathrm{inv}\left(\mathrm{FO}\oplus\mathbf{ARB}\right)$-Satz
definierbar (siehe dazu Abschnitt 4.4 von \cite{AMSS2012-locality}).

Sei $\varphi$ ein beliebiger $\mathrm{FO}+\mathbf{ARB}$-Satz mit
dem Quantorenrang $\mathrm{qr}\left(\varphi\right)=m$. Wir werden
zwei Strukturen $\mathfrak{A}\uplus\Upsilon\left(n\right)$ und $\mathfrak{B}\uplus\Upsilon\left(n\right)$
der gleichen Größe $m\geqslant$ durch ein Ehrenfeucht-Fraïssé-Spiel
(siehe Kapitel 2 von \cite{EbbinghausFlum}) anwenden, um zu zeigen,
dass 
\end{proof}
