...
X
\end{array}\right)\right)\\
F\left(X\right) & \coloneqq & q_{\varphi^{\beta}}\left(\mathfrak{A}\cup\mathfrak{R}_{X}\right)
\end{eqnarray*}
...
$F^{\infty}$ liegt: 
\begin{eqnarray*}
\left\llbracket \left[\mathrm{lfp}_{R,\bar{x}}\psi\right]\left(\bar{y}\right)\right\rrbracket \left(\mathfrak{A},\beta\right) & \coloneqq & \left\llbracket R\bar{y}\right\rrbracket \left(\mathfrak{A}\cup\mathfrak{R}_{F^{\infty}\left(\emptyset\right)},\beta\right)
\end{eqnarray*}
...
beschränktem Orakel in Polynomialzeit\cite{arora-barak}, und kann
daher genau solche Schaltkreisfamilien berechnen, deren binäre Repräsentation
polynomielle Größe hat. Unter Voraussetzung einer geeigneten Kodierung
sind dies genau die $\mathrm{poly}\left(n\right)$-großen Schaltkreisfamilien.\end{rem*}
\begin{defn}
...
\[
\begin{array}{ccccccc}
\left\llbracket y_{1}\dot{=}y_{2}\right\rrbracket \left(\mathfrak{A},\bar{t}\right)=1 & \Leftrightarrow & \left(\beta_{\bar{t}}y_{1}=\beta_{\bar{t}}y_{2}\right) & \Leftrightarrow & \mathcal{C}_{n}^{\varphi}\left[\mathfrak{A}\right]\left(g_{\bar{t}}\right) & = & \left\llbracket \mathcal{C}_{n}^{\varphi}\right\rrbracket \left(\mathfrak{A},\bar{t}\right)=1\end{array}
\]
...
zu $\Upsilon'\left(n\right)=\rho\Upsilon'\left(n\right)=\left(\left[n+1,2n+1\right],\left(\rho R^{\Upsilon\left(n\right)}\right)_{R\in\eta}\right)$.

Zunächst wird der $\left(\sigma\uplus\eta\uplus\left\{ \leqslant\right\} ,\mathbb{B}_{\mathrm{std}}\right)$-Schaltkreis
...
\subsection{Beliebige Orakel mit Zählterm}
...
und $\Upsilon:\mathbb{N}\rightarrow\mathbf{FIN}_{<}\left(\eta\right)$
ein Orakel.
...
\begin{proof}
Zunächst wird die Konstruktion aus Lemma \ref{lem:fo-c} angewendet,
um eine $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreisfamilie
...
Diese Schaltkreise haben die Tiefe $T_{n}\leqslant3\left\Vert \varphi\right\Vert $
und Größe $S_{n}\leqslant5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathtt{M}F\left(\hat{\varphi}\right)}\leqslant5\left\Vert \varphi\right\Vert \left(2n+1\right)^{\mathtt{M}F\left(\varphi\right)}$.

Dann werden die Konstruktionen aus Lemma \ref{lem:fo-ord} und Lemma
\ref{lem:fo-u} kombiniert, um daraus eine symmetrische $\left(\sigma,\mathbb{B}_{\mathrm{maj}}\right)$-Schaltkreisfamilie
