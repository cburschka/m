...
$P$-uniforme Schaltkreisklasse $\left(\mathrm{SBC}+\mathbf{MAJ}\right)^{P}$
...
\begin{eqnarray*}
\psi_{\mathrm{eval}}\left(\bar{u}\bar{x}\bar{v}\bar{m}\right) & \coloneqq & \bar{m}=\#\bar{y}\left(\mathrm{EVAL}\left(\bar{v}\bar{y}\right)\wedge\varphi_{\sim}\left(\bar{u}\bar{v}\bar{x}\bar{y}\right)\right)\\
