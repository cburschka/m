...
sind wie folgt definiert. Wir verwenden die Kal\end{defn}
...
diese wird auch durch $R/k\in\mathbf{var}_{2}$ notiert. (Per Definition
sei $\mathbf{var}_{2}$ disjunkt von allen hier betrachteten relationalen
Signaturen $\sigma$.)

Für eine Menge $\mathcal{R}\subseteq\mathbf{var}_{2}$ ist die $\mathrm{FO}\left[\sigma\cup\mathcal{R}\right]$-Logik
eine Erweiterung der $\mathrm{FO}\left[\sigma\right]$-Logik um die
folgende Regel:\end{defn}
\begin{lyxlist}{00.00.0000}
\item [{(AV)}] Für jede Relationsvariable $R/k\in\mathcal{R}$ und ein
$k$-Tupel $\bar{x}$ von $\mathrm{FO}\left[\sigma\cup\mathcal{R}\right]$-Termen
ist $R\bar{x}$ eine $\mathrm{FO}\left[\sigma\cup\mathcal{R}\right]$-Formel.
\end{lyxlist}

\begin{defn}
\textbf{Positivität}

Die Logik $\mathrm{FO}^{+}\left[\sigma\cup\mathcal{R}\right]$ sei
eine Einschränkung von $\mathrm{FO}\left[\sigma\right]$ auf Formeln
ohne die Negation (RN). Wir definieren die Klasse der $R$-positiven
$\mathrm{FO}\left[\sigma\right]$-Formeln wie folgt:\end{defn}
\begin{enumerate}
\item Für $R\in\mathbf{var}_{2}$ ist jede $\mathrm{FO}\left[\sigma\cup\mathcal{R}\backslash\left\{ R\right\} \right]$-Formel
$\varphi$ sowohl $R$-positiv als auch $R$-negativ, da sie $R$
nicht enthält.
\item Für $\mathcal{R}\subseteq\mathbf{var}_{2}$ und eine $R$-positive
$\mathrm{FO}\left[\sigma\cup\mathcal{R}\right]$-Formel $\varphi$
ist $\neg\varphi$ eine $R$-negative Formel, und umgekehrt.
\item Für einen Junktor $\gamma\in\left\{ \wedge,\vee\right\} $ und $k\geqslant2$
$R$-positive (beziehungsweise $R$-negative) Formeln $\varphi_{1},\cdots,\varphi_{k}$
ist $\left(\varphi_{1}\gamma\cdots\gamma\varphi_{k}\right)$ ebenfalls
$R$-positiv (beziehungsweise $R$-negativ).
\item Für einen Quantor $Q\in\left\{ \exists,\forall\right\} $, eine Variable
$x\in\mathbf{var}$ und eine $R$-positive (beziehungsweise $R$-negative)
Formel $\varphi$ ist $Qx\varphi$ ebenfalls $R$-positiv (beziehungsweise
$R$-negativ).
\end{enumerate}

\begin{defn}
Die inflationäre Fixpunktlogik $\mathrm{FO}+\mathrm{LFP}\left[\sigma\right]$
erweitert $\mathrm{FO}\left[\sigma\right]$ um den folgenden Fixpunkt-Operator:\end{defn}
\begin{lyxlist}{00.00.0000}
\item [{A}] Für $\mathcal{R}\subseteq\mathbf{var}_{2}$, ein $k$-Tupel
$\bar{x}$ von $\mathrm{FO}+\mathrm{LFP}\left[\sigma\cup\mathcal{R}\right]$-Termen
und eine Relationsvariable $R/k\in\mathbf{var}_{2}$ ist $R\bar{x}$
eine $\mathrm{FO}+\mathrm{IFP}\left[\sigma\cup\mathcal{R}\cup\left\{ R\right\} \right]$-Formel.
\item [{R4}] Für $\mathcal{R}_{1},\mathcal{R}_{2}\subseteq\mathbf{var}_{2}$,
eine $\mathrm{FO}+\mathrm{LFP}\left[\sigma\cup\mathcal{R}_{1}\right]$-Formel
$\psi$, eine Relationsvariable $R/k\in\mathbf{var}_{2}$, ein Tupel
$\bar{x}\in\mathbf{var}^{k}$ und ein $k$-Tupel $\bar{y}$ von $\mathrm{\mathrm{FO}+\mathrm{IFP}}\left[\sigma\cup\tau_{2}\right]$-Termen
ist $\varphi=\left[\mathrm{ifp}_{R,\bar{x}}\psi\right]\left(\bar{y}\right)$
eine $\mathrm{\mathrm{FO}+\mathrm{IFP}}\left[\sigma\cup\left(\tau_{1}\backslash\left\{ R\right\} \right)\cup\tau_{2}\right]$-Formel.
\begin{eqnarray*}
...
\end{lyxlist}
\begin{rem*}
Insbesondere besteht für eine relationale Signatur $\sigma$ die Logik
$\mathrm{FO}+\mathrm{IFP}\left[\sigma\right]$ nur aus Formeln, in
denen alle Relationsvariablen durch Fixpunkt-Operatoren gebunden sind.\end{rem*}
\begin{description}
\item [{Auswertung:}] Für eine $\sigma$-Struktur $\mathfrak{A}$, eine
$\mathrm{FO}+\mathrm{IFP}\left[\sigma\right]$-Formel und eine Belegung
$\beta:\mathrm{frei}\left(\varphi\right)\rightarrow A$ definieren
wir eine Sequenz von $\left\{ R\right\} $-Strukturen $\left(\mathfrak{R}^{\left(i\right)}\right)_{i\in\mathbb{N}}$
...
\end{example*}
Im folgenden beschränken wir uns ohne Beschränkung der Allgemeinheit
auf Formeln mit einem einzigen Fixpunktoperator.
\begin{fact}
Jede $\mathrm{FO}+\mathrm{IFP}\left[\sigma\right]$-Formel ist 
\end{fact}

\begin{fact}
Per \cite{Gurevich1986}, ist $\mathrm{IFP}$ in der Ausdrucksstärke
äquivalent zur Least-Fixed-Point-Logik $\mathrm{LFP}$ und der partiellen
Fixpunktlogik $\mathrm{PFP}.$ Im folgenden wird daher die Bezeichnung
$\mathrm{FP}$ und die inflationäre Definition verwendet.
\end{fact}
