...
Für $\eta=\emptyset$ und $\Upsilon_{\leqslant}\left(n\right)\coloneqq\left(\left[0,n\right],\leqslant_{\mid\left[0,n\right]}\right)\in\mathbf{FIN}_{<}\left(\emptyset\right)$
wird $\mathcal{L}+\Upsilon_{\leqslant}$ als $\mathcal{L}+\mathbf{ORD}$
bezeichnet.

Für $\eta=\left\{ \mathtt{BIT}\right\} $, wobei $\mathtt{BIT}\left(a,b\right)$
ausdrückt, dass das $b$te Bit der Binärdarstellung von $a$ den Wert
$1$ hat, wird $\mathcal{L}+\Upsilon_{\mathtt{BIT}}$ als $\mathcal{L}+\mathbf{BIT}$
bezeichnet. 
\begin{eqnarray*}
\mathtt{BIT} & \coloneqq & \left\{ \left(a,b\right)\in\mathbb{N}^{2}\mid a=\Sigma_{i=0}^{\left\lceil \log a\right\rceil }x_{i}2^{i},\,\bar{x}\in\left\{ 0,1\right\} ^{\left\lceil \log a\right\rceil },\,x_{b}=1\right\} \\
\Upsilon_{\mathtt{BIT}}\left(n\right) & \coloneqq & \left(\left[0,n\right],\leqslant_{\mid\left[0,n\right]},\mathtt{BIT}_{\mid\left[0,n\right]}\right)\in\mathbf{FIN}_{<}\left(\left\{ \mathtt{BIT}\right\} \right)
\end{eqnarray*}
...
Algorithmus berechnet. Sei $\varphi$ in einer geeigneten Form kodiert:
...
rigide, symmetrische, $P$-uniforme $\left(\sigma,\mathbb{B}\right)$-Multischaltkreisfamilie,
wobei $\mathbb{B}\in\left\{ \mathbb{B}_{\mathrm{std}},\mathbb{B}_{\mathrm{maj}}\right\} $.
Wir möchten eine $\left(\mathrm{LFP}+\mathbf{ORD}\right)\left[\sigma\right]$-Formel
beziehungsweise $\left(\mathrm{LFP}+C\right)\left[\sigma\right]$-Formel
$\varphi$ aufbauen, so dass für $n\in\mathbb{N}$ und $\mathfrak{A}\in\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$
die Formel die gleiche Anfrage definiert wie $\mathcal{C}_{n}$:
...
Da $\mathcal{C}$ $P$-uniform ist, existiert nach dem Immerman-Vardi-Theorem\cite{Vardi:1982:CRQ:800070.802186,IMMERMAN198686}
und Lemma \ref{lem:algo-rigide} eine Sammlung von $\mathrm{LFP}\left[\leqslant\right]$-Formeln
$\Phi$, die auf der Struktur $\Upsilon\left(n\right)\in\mathbf{FIN}_{<}\left(\sigma\right)$
ausgewertet den Schaltkreis $\mathcal{C}_{n}$ beschreiben:
\[
\Phi\coloneqq\left(\varphi_{G},\varphi_{\mathcal{W}},\varphi_{\Omega},\left(\varphi_{\phi}\right)_{\phi\in\mathbb{B}\uplus\left\{ \mathbf{0},\mathbf{1},\neg\right\} },\left(\varphi_{R}\right)_{R\in\sigma}\right)
\]
...
\begin{eqnarray*}
\Upsilon\left(n\right)\models\varphi_{G}\left[\bar{g}\right] & \Leftrightarrow & f_{G}^{-1}\left(\bar{g}\right)\in G\\
\Upsilon\left(n\right)\models\varphi_{W}\left[\bar{h}\bar{g}\bar{w}\right] & \Leftrightarrow & \mathcal{W}\left(f_{G}^{-1}\left(\bar{h}\right),f_{G}^{-1}\left(\bar{g}\right)\right)=f_{W}^{-1}\left(\bar{w}\right)\\
\Upsilon\left(n\right)\models\varphi_{\Omega}\left[\bar{t}\bar{g}\right] & \Leftrightarrow & \Omega\left(\bar{t}\right)=f_{G}^{-1}\left(\bar{g}\right)\\
 &  & \mathrm{f\ddot{u}r}\,\bar{t}\in\left[1,n\right]^{k}\\
\Upsilon\left(n\right)\models\varphi_{\phi}\left[\bar{g}\right] & \Leftrightarrow & \Sigma\left(f_{G}^{-1}\left(\bar{g}\right)\right)=\phi\\
 &  & \mathrm{f\ddot{u}r}\,\phi\in\mathbb{B}\uplus\left\{ \mathbf{0},\mathbf{1},\neg\right\} \\
\Upsilon\left(n\right)\models\varphi_{R}\left[\bar{g}\bar{x}\right] & \Leftrightarrow & \Sigma\left(f_{G}^{-1}\left(\bar{g}\right)\right)=R\bar{x}\\
 &  & \mathrm{f\ddot{u}r}\,R/m\in\sigma,\,\bar{x}\in\left[1,n\right]^{m}
\end{eqnarray*}


Für $n<n_{0}$ gibt es nur eine endliche Anzahl von Schaltkreisen
konstanter Tiefe und Größe $\mathcal{C}_{n}$. Jeder dieser Schaltkreise
ist durch eine $\mathrm{FO}\left[\sigma\right]$-Formel kodierbar:
Sei $\psi_{n}\left(\bar{x}\right)$ eine Formel, die prüft, ob $\left|A\right|=n$,
und dann für jede der $n!$ Bijektionen $\pi:A\rightarrow U$ prüft,
ob $\mathcal{C}\left[\pi\mathfrak{A}\right]\left(\Omega\left(\pi\beta\bar{x}\right)\right)$
mit $\beta:\bar{x}\rightarrow A$.

\pagebreak{}


\section{Konstante Tiefe}
