...
notiert.
\item Für eine geordnete Menge $U=\left\{ u_{1},\cdots,u_{k}\right\} $
sei $\bar{U}$ das Tupel $\left(u_{1},\cdots,u_{k}\right)$ mit $u_{1}<\cdots<u_{k}$.
\end{itemize}
\end{defn}
...
Für eine Funktion $f:U\rightarrow\mathbb{R}$ und eine Menge $X\subseteq U$
bezeichnen $\min_{x\in X}f\left(x\right)\in\mathbb{R}$ und $\max_{x\in X}f\left(x\right)\in\mathbb{R}$
...
(Die Signatur $\sigma$ kann in Ausnahmefällen unendlich sein; da
wir jedoch nur endliche $\sigma$-Anfragen betrachten, können diese
sich nur auf einer endlichen Menge von Relationssymbolen $\sigma'\subseteq_{\mathrm{fin}}\sigma$
beziehen.)
...
die endlichen $\sigma\cup\left\{ \leqslant\right\} $-Strukturen mit
dem Universum $\left[a,b\right]$, wobei $\leqslant$ durch die natürliche
Ordnung von $\left[a,b\right]$ interpretiert wird, und sei $\mathbf{FIN}_{\leqslant}^{a}\left(\sigma\right)\coloneqq\bigcup_{b\in\mathbb{N}}\mathbf{FIN}_{\leqslant}^{\left[a,b\right]}\left(\sigma\right)$
...
diese bilden eine Gruppe bezüglich der Verkettung $\circ$ und dem
neutralen Element $\mathbf{id}$.
\end{defn}
