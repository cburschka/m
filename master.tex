...
\end{enumerate}
Nach den Definitionen aus Kapitel 7 sind alle $\mathcal{P}_{\left(uv\right)}$
...
aufgenommen worden, und es gilt $\mathcal{P}_{\left(uv\right)}\preceq\mathcal{P}_{g}$.
...
Gates $g$ nur von der dem Teil der Abbildung $\pi$ abhängt, die
Elemente auf $\mathrm{sp}\left(g\right)$ abbildet. Die Abbildung
der übrigen Elemente ist für $g$ unbedeutend.
\begin{defn}
\textbf{Bildreduktion}
...
\begin{prop}
Sei $\mathcal{C}=\left(G,W,\Sigma,\Omega,U\right)$ ein rigider, symmetrischer
Schaltkreis mit $n=\left|U\right|$, und sei $g\in G$ ein beliebiges
Gate. Sei $\mathfrak{A}\in\mathbf{FIN}^{\left(n\right)}\left(\sigma\right)$
...
in $\mathrm{sp}\left(g\right)$ sind. So gilt $\mathcal{C}\left[\pi_{1}\mathfrak{A}\right]\left(g\right)=\mathcal{C}\left[\pi_{2}\mathfrak{A}\right]\left(g\right)$.\end{prop}
\begin{proof}
