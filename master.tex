...
und unbeschränkt große Schaltkreise von Martin Otto in 1997\cite{Otto1997}),
...
$\Phi$, die auf der Struktur $\mathbf{ORD}\left(n\right)=\left(\left[0,n\right],\leqslant_{\left[0,n\right]}\right)$
...
der höchstens $n^{c}$ Gates von $\mathcal{C}_{n}$, und $f_{W}:\left[1,n^{c}\right]\rightarrow\left[1,n\right]^{c}$
eine Kodierung von Zahlen, so dass für $\bar{g},\bar{h},\bar{w}\in\left[0,n\right]^{c}$
gilt:
\begin{eqnarray*}
\mathbf{ORD}\left(n\right)\models\varphi_{G}\left[\bar{g}\right] & \Leftrightarrow & f_{G}^{-1}\bar{g}\in G\\
\mathbf{ORD}\left(n\right)\models\varphi_{W}\left[\bar{h}\bar{g}\bar{w}\right] & \Leftrightarrow & \mathcal{W}\left(f_{G}^{-1}\bar{h},f_{G}^{-1}\bar{g}\right)=f_{W}^{-1}\bar{w}\\
\mathbf{ORD}\left(n\right)\models\varphi_{\Omega}\left[\bar{t}\bar{g}\right] & \Leftrightarrow & \Omega\bar{t}=f_{G}^{-1}\bar{g}\\
 &  & \mathrm{f\ddot{u}r}\,\bar{t}\in\left[1,n\right]^{k}\\
\mathbf{ORD}\left(n\right)\models\varphi_{\phi}\left[\bar{g}\right] & \Leftrightarrow & \Sigma\left(f_{G}^{-1}\bar{g}\right)=\phi\\
 &  & \mathrm{f\ddot{u}r}\,\phi\in\mathbb{B}\uplus\left\{ \mathbf{0},\mathbf{1},\mathtt{NOT}\right\} \\
\mathbf{ORD}\left(n\right)\models\varphi_{R}\left[\bar{g}\bar{x}\right] & \Leftrightarrow & \Sigma\left(f_{G}^{-1}\bar{g}\right)=R\bar{x}\\
...
Wir füllen alle Relationen $\bar{\mathrm{EV}}\left(g\right)$ auf
eine einheitliche Länge $d$ auf:
\[
\overline{\mathrm{EV}}\left(g\right)\coloneqq\left\{ \bar{a}\in A^{d}\mid\left(a_{1},\cdots,a_{\left|\mathrm{sp}\left(g\right)\right|}\right)\in\bar{\mathrm{EV}}\left(g\right),\,a_{i}\neq a_{j}\,\,\mathrm{f.a.}\,\,1\leqslant i<j\leqslant d\right\} 
\]


Nun definieren wir die Relation $\mathrm{EVAL}\subseteq\left[0,n\right]^{c}\times A^{d}$,
so dass $\bar{g}\bar{a}\in\mathrm{EVAL}$ genau dann $\bar{g}=f_{G}g$
...
$\mathrm{sp}\left(g\right)$ und Orbits in $P$. Daher existiert eine
$\mathrm{LFP}\left[\left\{ \leqslant\right\} \right]$-Formel $\varphi_{\mathrm{sp}}$,
so dass gilt:
\begin{eqnarray*}
\mathbf{ORD}\left(n\right)\models\varphi_{\mathrm{sp}}\left(\bar{g}u\right) & \Leftrightarrow & \mathbf{ORD}\left(n\right)\models\varphi_{G}\left[\bar{g}\right]\,\mathrm{und}\,u\in\mathrm{sp}\left(g\right)\\
 &  & \mathrm{f.a.}\,\bar{g}u\in\left[1,n\right]^{c+1}
\end{eqnarray*}
Ferner definieren wir die Ordnung der Elemente $\bar{\mathrm{sp}}\left(g\right)$
mit den folgenden Formeln:
\begin{eqnarray*}
\varphi_{\mathrm{sp},1}\left(\bar{x}y\right) & \coloneqq & \varphi_{\mathrm{sp}}\left(\bar{x}y\right)\wedge\forall z\left(\varphi_{\mathrm{sp}}\left(\bar{x}z\right)\rightarrow y\leqslant z\right)\\
\varphi_{\mathrm{sp},i+1}\left(\bar{x}y\right) & \coloneqq & \varphi_{\mathrm{sp}}\left(\bar{x}y\right)\wedge\exists z\left(\neg\left(\varphi_{\mathrm{sp},i}\left(\bar{x}z\right)\rightarrow y\leqslant z\right)\wedge\forall z'\left(\varphi_{\mathrm{sp}}\left(\bar{x}z'\right)\rightarrow\left(z'\leqslant z\vee y\leqslant z'\right)\right)\right)
\end{eqnarray*}


Für die Konsistenz von zwei Belegungen $\pi_{1}\coloneqq\left(\bar{\mathrm{sp}}\left(g\right)\mapsto\bar{a}\right)$
und $\pi_{2}\coloneqq\left(\bar{\mathrm{sp}}\left(h\right)\mapsto\bar{b}\right)$
definieren wir die folgende $\mathrm{LFP}+\mathbf{ORD}$-Formel, die
für $\beta\left(\bar{u}\bar{v}\bar{x}\bar{y}\right)$ entscheidet,
ob für die Abbildungen $\pi_{1}\coloneqq\left(\bar{\mathrm{sp}}\left(f_{G}^{-1}\beta\bar{u}\right)\mapsto\beta\bar{x}\right)$
und $\pi_{2}\coloneqq\left(\bar{\mathrm{sp}}\left(f_{G}^{-1}\beta\bar{v}\right)\mapsto\beta\bar{y}\right)$
gilt, dass $\pi_{1}\sim_{\mathrm{sp}\left(g\right)\cap\mathrm{sp}\left(h\right)}\pi_{2}$.
\begin{eqnarray*}
\varphi_{\sim}\left(\bar{u}\bar{v}\bar{x}\bar{y}\right) & \coloneqq & \bigwedge_{i,j\in\left[1,k\right]}\forall w\left(\left(\varphi_{\mathrm{sp},i}\left(\bar{u}w\right)\wedge\varphi_{\mathrm{sp},j}\left(\bar{v}w\right)\right)\rightarrow x_{i}=y_{j}\right)\\
...
die die Relation $\mathrm{EVAL}$ rekursiv definieren.
\begin{eqnarray*}
\psi_{\mathbf{0}}\left(\bar{u}\bar{x}\right) & \coloneqq & \neg x_{1}=x_{1}\\
\psi_{\mathbf{1}}\left(\bar{u}\bar{x}\right) & \coloneqq
\end{eqnarray*}
